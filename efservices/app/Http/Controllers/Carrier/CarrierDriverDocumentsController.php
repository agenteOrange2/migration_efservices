...
class CarrierDriverDocumentsController extends Controller
{
    /**
     * Display the documents tab for a specific driver
     */
    public function index(UserDriverDetail $driver)
    {
        try {
            ...
            return view('admin.drivers.list-driver.tabs.documents', compact(
                'driver',
                'documentsByCategory',
                'documentStats'
            ));
    ...
    public function downloadAll(UserDriverDetail $driver)
    {
        try {
            ...
            $user = auth()->user();
            if (!$user->carrierDetails || !$user->carrierDetails->carrier) {
                Log::warning('User does not have carrier details', [
                    'user_id' => $user->id,
                    'driver_id' => $driver->id
                ]);
            ...
            if ($driver->carrier_id != $carrier->id) {
                Log::warning('Unauthorized access attempt to download driver documents', [
                    'driver_id' => $driver->id,
            ...
            Log::info('Starting downloadAll process', ['driver_id' => $driver->id, 'carrier_id' => $carrier->id]);
    ...
    {
        try {
            // Verify that the user is a carrier
            $user = auth()->user();
            if (!$user->carrierDetails || !$user->carrierDetails->carrier) {
                Log::warning('Unauthorized access attempt to download selected driver documents', [
                    'user_id' => $user->id
                ]);
                abort(403, 'Unauthorized access.');
            }
    ...
    private function getVehicleVerificationDocuments(UserDriverDetail $driver)
    {
        ...
        if (class_exists('App\Models\VehicleVerificationToken')) {
            ...
            Log::info('Vehicle verification tokens found', ['count' => $verificationTokens->count()]);
