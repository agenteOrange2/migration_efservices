...
class CertificationStep extends Component
{
     ...
     * @param UserDriverDetail $userDriverDetail
     */
    public function generateApplicationPDFs(UserDriverDetail $userDriverDetail, $signature = null)
    {
     ...
     */
    public function generateW9WithSignature(UserDriverDetail $userDriverDetail, string $signature)
    {
        try {
     ...
     */
    public function generateDotPolicyWithSignature(UserDriverDetail $userDriverDetail, string $signature)
    {
        try {
     ...
     */
    public function generateLeaseAgreementPDF(UserDriverDetail $userDriverDetail, $signaturePath = null)
    {
        try {
            ...
            // Get vehicle information via activeVehicleAssignment
            $vehicle = $userDriverDetail->activeVehicleAssignment?->vehicle;
            
            // If no active assignment, try to get the most recent assignment
            if (!$vehicle) {
                $vehicle = $userDriverDetail->vehicleAssignments()->with('vehicle')->latest()->first()?->vehicle;
            }
            
            // Log vehicle data for debugging
            Log::info('CertificationStep - Vehicle data for third party consent:', [
     ...
     */
    public function generateCombinedPDF(UserDriverDetail $userDriverDetail, $signatureImage)
    {
        try {
     ...
     */
    public function prepareSignatureForPDF($signature)
    {
     ...
     * @param string $signaturePath Ruta al archivo de firma
     */
    public function generateThirdPartyDocuments(UserDriverDetail $userDriverDetail, $signaturePath = null)
    {
        try {
            // Cargar todas las relaciones necesarias para asegurar que tenemos los datos completos
            $userDriverDetail->load([
     ...
     * @return array
     */
    public function getEffectiveDates($driverId)
    {
     ...
     */
    public function generateCriminalHistoryPDF(UserDriverDetail $userDriverDetail, $signatureImage)
    {
        try {
