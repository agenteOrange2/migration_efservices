...
class DriverApplicationStep extends Component
{
    ...
    {
        $this->applicationService = app(DriverApplicationService::class);
    }
    ...
    // Cached lists (from CacheableListsTrait)
    public $cachedStates = [];
    ...
    protected function getRequiredRules(): array
    {
        ...
        // Company driver has no additional required fields
    ...
    protected function getOptionalRules(): array
    {
        ...
        // Add optional rules based on selected driver type
        if ($this->selectedDriverType === 'owner_operator') {
            $rules = array_merge($rules, [
     ...
     * Note: For admin editing, we allow proceeding regardless of current_step
     * since admins should be able to edit any step at any time
     */
    protected function validateStepCompletion()
    {
        if (!$this->driverId) {
            Log::warning('ApplicationStep: validateStepCompletion failed - no driverId');
        ...
        Log::info('ApplicationStep: validateStepCompletion check', [
            'driver_id' => $this->driverId,
            'driver_found' => $driver ? 'yes' : 'no',
            'driver_current_step' => $driver ? $driver->current_step : null,
        ]);
        
        if (!$driver) {
            Log::warning('ApplicationStep: validateStepCompletion failed - driver not found', [
                'driver_id' => $this->driverId,
            ]);
            $this->dispatch('notify', [
                'type' => 'error',
        ...
        // For admin context, allow proceeding regardless of current_step
        // The admin should be able to edit and save any step
        // The current_step will be updated after successful save
        Log::info('ApplicationStep: validateStepCompletion passed (admin context)');
        return true;
    }
    ...
    public function mount($driverId = null)
    {
        ...
        // Usar cache para listas de selección (CacheableListsTrait)
        $this->vehicleMakes = VehicleMake::orderBy('name')->get();
        $this->vehicleTypes = VehicleType::orderBy('name')->get();
        
        // Precargar listas cacheadas para uso en la vista
        $this->cachedStates = $this->getCachedStates();
    ...
    public function selectVehicle($vehicleId)
    {
        ...
        // Use int cast to handle type differences (int vs string)
    ...
    public function updatedSelectedDriverType($value)
    {
        ...
        if ($value === 'third') {
            $value = 'third_party';
            $this->selectedDriverType = $value; // Update the property with normalized value
        ...
        if ($value) {
            ...
            // Store the selected driver type before loading existing data
            $selectedType = $value;
            
            // Load existing data FIRST to preserve all data
            $this->loadExistingData();
            
            // CRITICAL: Restore the selected driver type after loading existing data
            // loadExistingData may reset selectedDriverType based on database state
            $this->selectedDriverType = $selectedType;
            ...
            $this->deletePreviousAssignments($selectedType);
            
            // Create new assignment for the selected type
            $this->createVehicleDriverAssignment($selectedType);
            
            Log::info('Vehicle assignment created for driver type', [
                'driver_type' => $selectedType,
                'driver_id' => $this->driverId,
                'selectedDriverType_after' => $this->selectedDriverType
            ]);
            
            // Auto-fill owner fields when owner_operator is selected (only if no existing data)
            if ($selectedType === 'owner_operator' && empty($this->owner_name)) {
    ...
    protected function saveApplicationDetails()
    {
        ...
        try {
            ...
            if ($this->selectedDriverType === 'owner_operator') {
                ...
                // Clean up other type assignments
    ...
    public function next()
    {
        ...
        // Progressive validation - validate only required fields (Requirement 3.1)
        // This allows advancing with optional fields empty (Requirement 3.2)
        try {
            Log::info('ApplicationStep: About to validate progressively', [
                'driver_id' => $this->driverId,
                'selectedDriverType' => $this->selectedDriverType,
                'applying_position' => $this->applying_position,
                'applying_location' => $this->applying_location,
            ...
            $this->validateProgressively();
            Log::info('ApplicationStep: Validation passed successfully');
        } catch (\Illuminate\Validation\ValidationException $e) {
            // Required field validation failed - cannot proceed
            Log::error('ApplicationStep: Required field validation failed', [
                'driver_id' => $this->driverId,
                'errors' => $e->errors(),
                'selectedDriverType' => $this->selectedDriverType,
            ...
            Log::error('ApplicationStep: Unexpected error during validation', [
                'driver_id' => $this->driverId,
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);
        ...
        // Save to database
        if ($this->driverId) {
            Log::info('ApplicationStep: About to save application', ['driver_id' => $this->driverId]);
            if (!$this->saveApplicationWithMultipleTypes()) {
                Log::error('ApplicationStep: Save failed, stopping next()', ['driver_id' => $this->driverId]);
                return; // Stop if save failed
            }
            Log::info('ApplicationStep: Save completed successfully', ['driver_id' => $this->driverId]);
        }

        // Move to next step
        Log::info('ApplicationStep: Dispatching nextStep event', ['driver_id' => $this->driverId]);
    ...
    protected function performAutoSave(): void
    {
        ...
        try {
            // Guardar solo los datos básicos de la aplicación sin validación completa
            $userDriverDetail = UserDriverDetail::find($this->driverId);
            if (!$userDriverDetail) {
                return;
            }

            // Actualizar o crear la aplicación con datos parciales
            ...
            Log::info('AutoSave completed for ApplicationStep', [
                'driver_id' => $this->driverId,
            ]);

        } catch (\Exception $e) {
            Log::error('AutoSave failed for ApplicationStep', [
                'driver_id' => $this->driverId,
                'error' => $e->getMessage(),
            ]);
            throw $e;
