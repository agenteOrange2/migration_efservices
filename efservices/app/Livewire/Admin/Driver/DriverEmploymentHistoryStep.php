...
class DriverEmploymentHistoryStep extends Component
{
    ...
    protected EmploymentHistoryService $employmentService;
    ...
    // Employment Summary (from service)
    public $employmentSummary = [];
    public $employmentGaps = [];
    ...
    public $showVerificationTokenModal = false;
    public $selectedCompanyForTokenView = null;
    ...
    {
        $this->employmentService = app(EmploymentHistoryService::class);
    }

    /**
     * Perform auto-save (required by AutoSaveTrait)
     */
    protected function performAutoSave(): void
    {
        ...
        try {
            ...
            Log::info('Auto-save completed for employment history', [
                'driver_id' => $this->driverId,
            ]);
        } catch (\Exception $e) {
            Log::error('Auto-save failed for employment history', [
                'driver_id' => $this->driverId,
                'error' => $e->getMessage(),
            ]);
            throw $e;
    ...
    /**
     * Cargar resumen de empleo usando el servicio
     */
    protected function loadEmploymentSummary(): void
    {
        if (!$this->driverId) {
            return;
        }
        
        $this->employmentSummary = $this->employmentService->getEmploymentSummary($this->driverId);
        $this->employmentGaps = $this->employmentService->getEmploymentGaps($this->driverId);
        $this->years_of_history = $this->employmentService->calculateYearsOfHistory($this->driverId);
    }
    ...
    public function sendVerificationEmail($companyId)
    {
        ...
        DB::beginTransaction();
        try {
            // Find the employment company with master company
            $employmentCompany = DriverEmploymentCompany::with('masterCompany')->find($companyId);
            ...
            // Get master company
            $masterCompany = $employmentCompany->masterCompany;
            if (!$masterCompany) {
                throw new \Exception('Master company not found');
            }

            // Get the email - prioritize MasterCompany email, fallback to DriverEmploymentCompany email
            $emailToUse = !empty($masterCompany->email) ? $masterCompany->email : $employmentCompany->email;
            ...
            // Sync the email to DriverEmploymentCompany if it's different
            if ($employmentCompany->email !== $emailToUse) {
                $employmentCompany->update(['email' => $emailToUse]);
                Log::info('Synced email from MasterCompany to DriverEmploymentCompany', [
                    'company_id' => $companyId,
                    'old_email' => $employmentCompany->email,
                    'new_email' => $emailToUse
                ]);
            }
            ...
            // Count attempt number (all tokens for this company)
            ...
            session()->flash('success', 'Verification email sent successfully to ' . $masterCompany->company_name . ' (Attempt #' . $attemptNumber . ')');
    ...
    public function resendVerificationEmail($companyId)
    {
        ...
        try {
            ...
            $existingToken = EmploymentVerificationToken::where('driver_id', $this->driverId)
                ->where('employment_company_id', $companyId)
            ...
            // Always generate a new token when resending to ensure the new email is used
            // This prevents issues where the old token has the old email stored
            $token = Str::random(64);
            
            // Create new employment verification token record with the CURRENT email
            EmploymentVerificationToken::create([
                'driver_id' => $this->driverId,
                'employment_company_id' => $employmentCompany->id,
                'token' => $token,
            ...
            Log::info('Generated new verification token for resend', [
                'company_id' => $companyId,
                'email' => $emailToUse,
                'old_token_existed' => $existingToken ? true : false
            ]);
            ...
            session()->flash('success', 'Verification email resent successfully to ' . $masterCompany->company_name . ' (Attempt #' . $attemptNumber . ')');
    ...
    /**
     * Admin: View verification token status for a specific company
     * 
     * @param int $companyId The employment company ID
     * @return void
     */
    public function viewVerificationTokens($companyId)
    {
        if (!$this->driverId) {
            session()->flash('error', 'Driver ID not found');
            return;
        ...
        Log::info('Admin viewing verification tokens', [
            'driver_id' => $this->driverId,
            'company_id' => $companyId,
            'admin_action' => true
        ]);
        ...
        try {
            ...
            $this->selectedCompanyForTokenView = [
                'id' => $employmentCompany->id,
                'company_name' => $employmentCompany->masterCompany ? $employmentCompany->masterCompany->company_name : 'Unknown Company',
                'email' => $employmentCompany->email,
                'email_sent' => $employmentCompany->email_sent ?? false
            ];

            // Load all verification tokens for this company
            $this->verificationTokens = EmploymentVerificationToken::where('driver_id', $this->driverId)
                ->where('employment_company_id', $companyId)
                ->orderBy('created_at', 'desc')
                ->get()
                ->map(function ($token) {
                    return [
                        'id' => $token->id,
                        'token' => substr($token->token, 0, 10) . '...', // Show partial token for security
                        'full_token' => $token->token,
                        'email' => $token->email,
                        'created_at' => $token->created_at->format('Y-m-d H:i:s'),
                        'expires_at' => $token->expires_at ? $token->expires_at->format('Y-m-d H:i:s') : 'N/A',
                        'is_expired' => $token->isExpired(),
                        'verified' => $token->verified ?? false,
                        'verified_at' => $token->verified_at ? $token->verified_at->format('Y-m-d H:i:s') : null,
                        'status' => $token->verified ? 'Verified' : ($token->isExpired() ? 'Expired' : 'Active')
                    ];
                })
                ->toArray();

            $this->showVerificationTokenModal = true;

            Log::info('Loaded verification tokens for admin view', [
                'driver_id' => $this->driverId,
                'company_id' => $companyId,
                'token_count' => count($this->verificationTokens)
            ]);
            
        } catch (\Exception $e) {
            Log::error('Error loading verification tokens', [
                'company_id' => $companyId,
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
                'driver_id' => $this->driverId
            ]);
            session()->flash('error', 'Error loading verification tokens: ' . $e->getMessage());
        }
    }

    /**
     * Admin: Close verification token modal
     * 
     * @return void
     */
    public function closeVerificationTokenModal()
    {
        $this->showVerificationTokenModal = false;
        $this->selectedCompanyForTokenView = null;
        $this->verificationTokens = [];
    }

    /**
     * Admin: Override email_sent flag for a specific company
     * Allows admin to manually mark email as sent or not sent
     * 
     * @param int $companyId The employment company ID
     * @param bool $status The new email_sent status
     * @return void
     */
    public function adminOverrideEmailStatus($companyId, $status)
    {
        if (!$this->driverId) {
            session()->flash('error', 'Driver ID not found');
            return;
        ...
        Log::info('Admin overriding email_sent status', [
            'driver_id' => $this->driverId,
            'company_id' => $companyId,
            'new_status' => $status,
            'admin_action' => true
        ...
        try {
            ...
            // Get master company for logging
            $masterCompany = MasterCompany::find($employmentCompany->master_company_id);
            $companyName = $masterCompany ? $masterCompany->company_name : 'Unknown Company';

            // Update email_sent flag
            $employmentCompany->update(['email_sent' => $status]);
            ...
            Log::info('Admin successfully overrode email_sent status', [
                'company_id' => $employmentCompany->id,
            ...
            $statusText = $status ? 'sent' : 'not sent';
            session()->flash('success', "Email status for {$companyName} has been marked as {$statusText}");
            
        } catch (\Exception $e) {
            DB::rollBack();
            Log::error('Error overriding email_sent status', [
                'company_id' => $companyId,
                'status' => $status,
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ...
            session()->flash('error', 'Error updating email status: ' . $e->getMessage());
        }
    }
