/**
 * Carrier Driver Testing Form Management
 * Adapted for carrier area - no carrier selection needed
 * Last updated: 2024-12-10 - Fixed API endpoint path
 */
class CarrierDriverTestingForm {
    constructor(options = {}) {
        ...
        // Auto-load drivers for authenticated carrier
        this.loadDrivers();
    }

    init() {
        // Method for backward compatibility
        console.log('CarrierDriverTestingForm initialized');
    }
    ...
    /**
     * Load active drivers for the authenticated carrier via API
     * 
     * This method fetches drivers from the carrier-specific API endpoint.
     * No carrier ID needed as it uses the authenticated carrier.
     */
    async loadDrivers() {
        // Show loading state
        ...
        try {
            ...
            const response = await fetch('/carrier/carrier-driver-testings/api/drivers', {
            ...
            if (!response.ok) {
                if (response.status >= 500) {
                    ...
                    throw new Error('Drivers endpoint not found.');
            ...
            if (Array.isArray(data)) {
                if (data.length > 0) {
                    ...
                    // If in edit mode, select the current driver
                    ...
                    // Handle empty driver list with warning
                    this.clearDriverSelect();
                    this.showWarning('No active drivers found for your carrier');
                }
            } else {
            ...
            // Differentiate between timeout, network, and server errors
    ...
    populateDriverSelect(drivers) {
        ...
        drivers.forEach(driver => {
            ...
            // Get license info from licenses array
    ...
    formatDriverName(data) {
        // Case 1: If data already has full_name property
    ...
    validateForm() {
        ...
        // Update hidden field
        if (this.userDriverDetailIdHidden) {
            this.userDriverDetailIdHidden.value = driverId;
        }
...
window.CarrierDriverTestingForm = CarrierDriverTestingForm;
