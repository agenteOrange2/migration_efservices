...
class CarrierVehicleMaintenanceEditTest extends TestCase
{
    use RefreshDatabase;
    ...
    /** @test */
    public function carrier_can_access_edit_page_for_their_vehicle_maintenance()
    {
        $response = $this->actingAs($this->user)
            ->get(route('carrier.vehicles.maintenance.edit', [$this->vehicle, $this->maintenance]));

        $response->assertStatus(200);
        $response->assertViewIs('carrier.vehicles.maintenance.edit');
    ...
    public function carrier_can_update_maintenance_record()
    {
        ...
        $response = $this->actingAs($this->user)
            ->put(route('carrier.vehicles.maintenance.update', [$this->vehicle, $this->maintenance]), $updateData);

        $response->assertRedirect(route('carrier.vehicles.show', $this->vehicle->id));
    ...
    public function carrier_cannot_access_edit_page_for_other_carriers_maintenance()
    {
        ...
        $response = $this->actingAs($this->user)
            ->get(route('carrier.vehicles.maintenance.edit', [$otherVehicle, $otherMaintenance]));
    ...
    public function carrier_cannot_update_other_carriers_maintenance()
    {
        ...
        $response = $this->actingAs($this->user)
            ->put(route('carrier.vehicles.maintenance.update', [$otherVehicle, $otherMaintenance]), $updateData);
    ...
    public function update_requires_authentication()
    {
        ...
        $response = $this->put(route('carrier.vehicles.maintenance.update', [$this->vehicle, $this->maintenance]), $updateData);
    ...
    {
        $response = $this->actingAs($this->user)
            ->put(route('carrier.vehicles.maintenance.update', [$this->vehicle, $this->maintenance]), []);

        $response->assertSessionHasErrors(['unit', 'service_tasks', 'service_date', 'vendor_mechanic', 'cost', 'odometer']);
    }
