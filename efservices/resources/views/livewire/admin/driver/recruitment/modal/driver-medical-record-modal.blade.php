<!-- Modal para subir Record Médico -->
<div x-data="{ open: false }" 
     x-init="$wire.on('open-medical-record-modal', () => { open = true });
             $wire.on('close-medical-record-modal', () => { open = false });"
     x-show="open" 
     x-cloak 
    ...
    <div x-show="open" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 transform translate-y-4"
        ...
        <div class="bg-white rounded-lg overflow-hidden max-w-lg w-full">
            <div class="px-4 py-5 sm:px-6 border-b border-slate-200 flex justify-between items-center">
                <h3 class="text-lg font-medium text-slate-900">Upload Medical Record</h3>
                <button @click="$dispatch('close-medical-record-modal')" class="text-slate-400 hover:text-slate-600">
            ...
            <div class="px-4 py-3 bg-slate-50 text-right sm:px-6 flex justify-end space-x-2">
                <button type="button" @click="$dispatch('close-medical-record-modal')"
                ...
                <button type="button" wire:click="uploadMedicalRecord"
