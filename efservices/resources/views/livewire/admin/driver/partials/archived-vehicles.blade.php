                        ...
                        <div class="flex items-start justify-between mb-4">
                            <div>
                                <h4 class="text-base font-semibold text-slate-900">
                                    @if(isset($assignment['unit_number']) && $assignment['unit_number'])
                                        Unit #{{ $assignment['unit_number'] }}
                                    @elseif(isset($assignment['vehicle_number']))
                                        {{ $assignment['vehicle_number'] }}
                                    @else
                                        Vehicle Assignment
                                    @endif
                                </h4>
                                @if((isset($assignment['vehicle_make']) && $assignment['vehicle_make']) || (isset($assignment['vehicle_model']) && $assignment['vehicle_model']))
                                    <p class="text-sm text-slate-600 mt-1">
                                        {{ $assignment['vehicle_make'] ?? '' }} {{ $assignment['vehicle_model'] ?? '' }}
                                        @if(isset($assignment['vehicle_year']) && $assignment['vehicle_year'])
                                            ({{ $assignment['vehicle_year'] }})
                                        @endif
                                    </p>
                                @endif
                                @if(isset($assignment['driver_type']) && $assignment['driver_type'])
                                    <p class="text-xs text-slate-500 mt-1">
                                        Driver Type: {{ ucfirst(str_replace('_', ' ', $assignment['driver_type'])) }}
                                    </p>
                                @endif
                            </div>
