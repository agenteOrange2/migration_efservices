...
<div class="space-y-8">
                ...
                @foreach($convictions as $conviction)
                    <div class="border border-slate-200 rounded-lg p-5 bg-slate-50/50">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <!-- Penalty -->
                            @if(isset($conviction['penalty']) && !empty($conviction['penalty']))
                            <div>
    ...
    <div class="border-t border-slate-200/60 pt-8">
        <h3 class="text-lg font-semibold text-slate-800 mb-4">DOT Inspections</h3>
                    ...
                    <div class="box box--stacked p-6">
                        <div class="flex items-start justify-between mb-6">
                            <div>
                                <h4 class="text-lg font-semibold text-slate-800">
                                    {{ $inspection['level'] ?? $inspection['inspection_level'] ?? 'DOT Inspection' }}
                                </h4>
                            ...
                            @if(isset($inspection['result']) || isset($inspection['status']))
                                @php
                                    ...
                                    $resultColor = $resultColors[strtolower($result)] ?? 'bg-slate-100 text-slate-700';
                                @endphp
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div class="bg-slate-50/50 rounded-lg p-4 border border-slate-100">
                                <label class="text-xs font-medium text-slate-500 uppercase tracking-wide mb-2 block">Location</label>
                                <p class="text-sm font-semibold text-slate-800">{{ $inspection['location'] }}</p>
                            </div>
                            @endif

                            @if(isset($inspection['inspector_name']))
                            <div class="bg-slate-50/50 rounded-lg p-4 border border-slate-100">
                                <label class="text-xs font-medium text-slate-500 uppercase tracking-wide mb-2 block">Inspector</label>
                                <p class="text-sm font-semibold text-slate-800">{{ $inspection['inspector_name'] }}</p>
                            </div>
                            @endif

                            @if(isset($inspection['violations']) || isset($inspection['defects_found']))
                            <div class="bg-slate-50/50 rounded-lg p-4 border border-slate-100">
                                <label class="text-xs font-medium text-slate-500 uppercase tracking-wide mb-2 block">Violations</label>
                                <p class="text-sm font-semibold text-slate-800">{{ $inspection['violations'] ?? $inspection['defects_found'] ?? '0' }}</p>
                            </div>
                            @endif
                        </div>
                    </div>
        ...
        @else
            <div class="text-center py-8 border border-slate-200 rounded-lg">
                <x-base.lucide class="h-10 w-10 text-slate-300 mx-auto mb-2" icon="ClipboardCheck" />
                <p class="text-slate-500 text-sm">No inspection records available</p>
            </div>
        @endif
    </div>
</div>
