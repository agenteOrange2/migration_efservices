<div class="bg-white p-4 rounded-lg shadow">
                                    ...
                                    @if ($item['type'] == 'employed')
                                        ...
                                        @if ($hasEmail && $companyId)
                                            ...
                                            <x-base.button type="button" variant="outline-secondary"
                                                wire:click="viewVerificationTokens({{ $companyId }})"
                                                class="text-blue-600 hover:text-blue-800 text-sm flex items-center gap-1"
                                                title="View verification token status">
                                            ...
                                            @if ($emailSent)
                                                <x-base.button type="button" variant="outline-warning"
                                                    wire:click="adminOverrideEmailStatus({{ $companyId }}, false)"
                                                    class="text-orange-600 hover:text-orange-800 text-sm flex items-center gap-1"
                                                    title="Mark as not sent"
                                                    wire:loading.attr="disabled"
                                                    wire:target="adminOverrideEmailStatus({{ $companyId }}, false)">
                                                    ...
                                                    <span wire:loading.remove wire:target="adminOverrideEmailStatus({{ $companyId }}, false)">Unsent</span>
                                                    <span wire:loading wire:target="adminOverrideEmailStatus({{ $companyId }}, false)">
                                                        <i class="fas fa-spinner fa-spin"></i>
                                                    </span>
                                                </x-base.button>
                                            @else
                                                <x-base.button type="button" variant="outline-success"
                                                    wire:click="adminOverrideEmailStatus({{ $companyId }}, true)"
                                                    class="text-green-600 hover:text-green-800 text-sm flex items-center gap-1"
                                                    title="Mark as sent"
                                                    wire:loading.attr="disabled"
                                                    wire:target="adminOverrideEmailStatus({{ $companyId }}, true)">
                                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4"><polyline points="20 6 9 17 4 12"/></svg>
                                                    <span wire:loading.remove wire:target="adminOverrideEmailStatus({{ $companyId }}, true)">Mark Sent</span>
                                                    <span wire:loading wire:target="adminOverrideEmailStatus({{ $companyId }}, true)">
                                                        <i class="fas fa-spinner fa-spin"></i>
                                                    </span>
                                                </x-base.button>
                                            @endif
    ...
    <!-- Admin: Verification Token Modal -->
    @if ($showVerificationTokenModal && $selectedCompanyForTokenView)
        <div class="modal group bg-gradient-to-b from-theme-1/50 via-theme-2/50 to-black/50 transition-[visibility,opacity] w-screen h-screen fixed left-0 top-0 [&:not(.show)]:duration-[0s,0.2s] [&:not(.show)]:delay-[0.2s,0s] [&:not(.show)]:invisible [&:not(.show)]:opacity-0 [&.show]:visible [&.show]:opacity-100 [&.show]:duration-[0s,0.4s] overflow-y-auto show z-50">
            <div class="w-[90%] mx-auto bg-white relative rounded-md shadow-md transition-[margin-top,transform] duration-[0.4s,0.3s] -mt-4 group-[.show]:mt-16 group-[.modal-static]:scale-[1.05] sm:w-[900px] p-6">
                <div class="flex justify-between items-center mb-4 border-b pb-3">
                    <div>
                        <h3 class="text-lg font-semibold text-gray-900">Verification Token Status</h3>
                        <p class="text-sm text-gray-600 mt-1">
                            Company: <strong>{{ $selectedCompanyForTokenView['company_name'] }}</strong>
                        </p>
                        <p class="text-sm text-gray-600">
                            Email: <strong>{{ $selectedCompanyForTokenView['email'] ?? 'N/A' }}</strong>
                        </p>
                        <p class="text-sm text-gray-600">
                            Status: 
                            @if ($selectedCompanyForTokenView['email_sent'])
                    ...
                    <button type="button" wire:click="closeVerificationTokenModal" class="text-gray-400 hover:text-gray-600">
                ...
                <div class="mb-4">
                    @if (count($verificationTokens) > 0)
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach ($verificationTokens as $token)
                                        <tr class="{{ $token['verified'] ? 'bg-green-50' : ($token['is_expired'] ? 'bg-red-50' : '') }}">
                                            <td class="px-4 py-3 text-sm text-gray-900">
                                                <code class="bg-gray-100 px-2 py-1 rounded text-xs">{{ $token['token'] }}</code>
                                            </td>
                                            <td class="px-4 py-3 text-sm text-gray-600">{{ $token['email'] }}</td>
                                            <td class="px-4 py-3 text-sm text-gray-600">{{ $token['created_at'] }}</td>
                                            <td class="px-4 py-3 text-sm text-gray-600">{{ $token['expires_at'] }}</td>
                                            ...
                                            <td class="px-4 py-3 text-sm text-gray-600">
                                                {{ $token['verified_at'] ?? 'Not verified' }}
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                        ...
                        <div class="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
                            <p class="text-xs text-blue-800">
                                <i class="fas fa-info-circle mr-1"></i>
                                <strong>Admin Note:</strong> This view shows all verification tokens generated for this company. 
                                Tokens are valid for 30 days from creation. Once verified, the token status will show as "Verified".
                            </p>
                        </div>
                    @else
                        <div class="text-center py-8">
                            ...
                            <p class="mt-2 text-sm text-gray-600">No verification tokens found for this company.</p>
                            <p class="mt-1 text-xs text-gray-500">Tokens are created when verification emails are sent.</p>
                        </div>
                    @endif
                </div>

                <div class="flex justify-end space-x-2 mt-4 pt-4 border-t">
                    <x-base.button type="button" wire:click="closeVerificationTokenModal" variant="outline-secondary">
                        Close
                    </x-base.button>
                </div>
            </div>
