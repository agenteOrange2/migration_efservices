...
@section('subcontent')
    ...
    <div class="box box--stacked p-8 mb-8">
        <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
            <div class="flex flex-col w-full md:flex-row text-center md:text-left items-center gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-bold text-slate-800 mb-2">Medical Record Documents</h1>
                    <p class="text-slate-600">Documents for {{ $medicalRecord->driverDetail->user->name ?? 'N/A' }}
                        {{ $medicalRecord->driverDetail->middle_name ?? 'N/A' }} {{ $medicalRecord->driverDetail->last_name ?? 'N/A' }}
                    </p>
                </div>
            </div>
    ...
    <div class="grid grid-cols-3 gap-5 mb-6">
        ...
        <a href="{{ route('admin.medical-records.docs.show', ['medicalRecord' => $medicalRecord->id, 'collection' => 'all'] + request()->except('collection', 'page')) }}"
            ...
            <div class="text-base {{ $currentCollection == 'all' ? 'text-primary' : 'text-slate-500' }}">Total Documents
            </div>
        ...
        <a href="{{ route('admin.medical-records.docs.show', ['medicalRecord' => $medicalRecord->id, 'collection' => 'medical_card'] + request()->except('collection', 'page')) }}"
            ...
            <div class="text-base {{ $currentCollection == 'medical_card' ? 'text-primary' : 'text-slate-500' }}">Medical
                Card</div>
        ...
        <a href="{{ route('admin.medical-records.docs.show', ['medicalRecord' => $medicalRecord->id, 'collection' => 'medical_documents'] + request()->except('collection', 'page')) }}"
            ...
            <div class="text-base {{ $currentCollection == 'medical_documents' ? 'text-primary' : 'text-slate-500' }}">
                Medical Documents</div>
            <div class="mt-1.5 text-2xl font-medium">{{ $additionalDocuments }}</div>
            <div class="absolute inset-y-0 right-0 mr-5 flex flex-col justify-center">
                <div
                    class="flex items-center rounded-full border border-secondary/10 bg-secondary/10 py-[2px] pl-[7px] pr-1 text-xs font-medium text-secondary">
    ...
    <div class="box p-5 mb-6">
        <form method="GET" action="{{ route('admin.medical-records.docs.show', $medicalRecord->id) }}"
            ...
            <div class="flex flex-col gap-4 sm:flex-row lg:flex-row">
                ...
                <div class="relative">
                    ...
                    <x-base.litepicker class="w-full sm:w-64" id="date-range-filter" name="date_range"
                        value="{{ request('date_range') }}" data-single-mode="false" placeholder="Select date range" />
                </div>

                <!-- Document Type Filter -->
                <div class="relative">
                    <div class="flex flex-col items-start md:flex-row md:items-center gap-3 ">
                        ...
                        <x-base.tom-select class="w-full sm:w-48" id="document-type-filter" name="document_type"
                            data-placeholder="All Types">
                            <option value="">All Types</option>
                            <option value="medical_card"
                                {{ request('document_type') == 'medical_card' ? 'selected' : '' }}>Medical Card</option>
                            <option value="medical_documents"
                                {{ request('document_type') == 'medical_documents' ? 'selected' : '' }}>Medical Documents
                            </option>
                        </x-base.tom-select>
                    </div>
                </div>
            ...
            <x-base.table class="border-b border-dashed border-slate-200/80">
                <x-base.table.thead>
                    <x-base.table.tr>
                        ...
                        <x-base.table.td
                            class="border-b-0 whitespace-nowrap bg-slate-50 py-4 font-medium text-slate-500 text-center">
                            Actions
                        </x-base.table.td>
                    </x-base.table.tr>
                </x-base.table.thead>
                        ...
                        <x-base.table.tr class="hover:bg-slate-50/50">
                            <x-base.table.td class="border-dashed py-4">
                                <div class="flex items-center">
                                    <div
                                        class="image-fit zoom-in h-10 w-10 overflow-hidden rounded-lg border border-slate-200/70">
                                        @if (in_array(strtolower(pathinfo($document->file_name, PATHINFO_EXTENSION)), ['jpg', 'jpeg', 'png', 'gif', 'webp']))
                                            <img alt="Document" class="tooltip cursor-pointer rounded-lg"
                                                src="{{ route('admin.medical-records.doc.preview', $document->id) }}"
                                    ...
                                    <div class="ml-4">
                                        <div class="font-medium text-slate-800">{{ Str::limit($document->file_name, 30) }}
                                        </div>
                            ...
                            <x-base.table.td class="border-dashed py-4">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('admin.medical-records.doc.preview', $document->id) }}"
                                        target="_blank"
                                    ...
                                    <form method="POST"
                                        action="{{ route('admin.medical-records.doc.delete', $document->id) }}"
                                        class="inline"
