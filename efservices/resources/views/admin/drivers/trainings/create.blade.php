        ...
        <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 lg:gap-6">
            <div class="flex flex-col w-full md:flex-row text-center md:text-left items-center gap-3 lg:gap-4">
                <div class="p-2 sm:p-3 bg-primary/10 rounded-xl border border-primary/20">
                    <x-base.lucide class="w-6 h-6 sm:w-8 sm:h-8 text-primary" icon="PlusCircle" />
                </div>
                <div>
                    <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-slate-800 mb-1 sm:mb-2">Create New Training
                    </h1>
                    <p class="text-sm sm:text-base text-slate-600">Add a new training for drivers</p>
                </div>
            </div>
                ...
                <form action="{{ route('admin.trainings.store') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="col-span-2">
                            ...
                            <x-base.form-input type="text" name="title" id="title" value="{{ old('title') }}"
                                required />
                            @error('title')
                        ...
                        <div class="col-span-2">
                            ...
                            <x-base.form-textarea name="description" id="description"
                                rows="4">{{ old('description') }}</x-base.form-textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="col-span-2" x-show="contentType === 'video'">
                            ...
                            <x-base.form-input type="url" name="video_url" id="video_url"
                                value="{{ old('video_url') }}" x-bind:required="contentType === 'video'"
                                placeholder="https://www.youtube.com/watch?v=..." />
                            <p class="mt-1 text-sm text-gray-500">insert the URL of YouTube, Vimeo or another video platform
                            </p>
                            @error('video_url')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="col-span-2" x-show="contentType === 'file'">
                            <div class="mb-4">
                                <x-base.form-label for="files"
                                    x-bind:required="contentType === 'file'">Files</x-base.form-label>
