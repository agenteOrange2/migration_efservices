...
@section('title', 'Edit License - EF Services')
...
<form id="licenseForm" action="{{ route('driver.licenses.update', $license) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <x-base.form-select id="license_class" name="license_class" required>
                        ...
                        @foreach(['A', 'B', 'C', 'D'] as $class)
                            <option value="{{ $class }}" {{ old('license_class', $license->license_class) == $class ? 'selected' : '' }}>
                                Class {{ $class }}
                            </option>
                        @endforeach
                    </x-base.form-select>
                ...
                <div>
                    ...
                    <x-base.form-select id="state_of_issue" name="state_of_issue" required>
                        ...
                        @foreach($states as $code => $name)
                            <option value="{{ $code }}" {{ old('state_of_issue', $license->state_of_issue) == $code ? 'selected' : '' }}>
                                {{ $name }}
                            </option>
                        @endforeach
                    </x-base.form-select>
                ...
                <div>
                    ...
                    <x-base.litepicker id="expiration_date" name="expiration_date" 
                        value="{{ old('expiration_date', $license->expiration_date?->format('m/d/Y')) }}" 
                ...
                <div class="sm:col-span-2">
                    ...
                    <x-base.form-input type="text" id="restrictions" name="restrictions" 
                        value="{{ old('restrictions', $license->restrictions) }}" placeholder="Enter any restrictions" />
                </div>
            </div>
        </div>
        ...
        <div class="box box--stacked p-6">
            ...
            <div class="space-y-6">
                ...
                @php
                    $selectedEndorsements = old('endorsements', $license->endorsements->pluck('id')->toArray());
                @endphp
                <div id="cdl_endorsements" class="{{ old('is_cdl', $license->is_cdl) ? '' : 'hidden' }} mt-6 py-6 border-t border-b border-slate-200">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div class="flex items-center gap-3">
                            <input id="endorsement_{{ $endorsement->id }}" name="endorsements[]" type="checkbox" 
                                ...
                                {{ in_array($endorsement->id, $selectedEndorsements) ? 'checked' : '' }}
                                class="w-5 h-5 rounded border-slate-300 text-primary focus:ring-primary focus:ring-offset-0" />
        ...
        <div class="box box--stacked p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    <x-base.form-label>Front of License</x-base.form-label>
                    @if($license->getFirstMediaUrl('license_front'))
                    <div class="mb-3">
                        <img src="{{ $license->getFirstMediaUrl('license_front') }}" 
                             alt="Current front" class="max-w-[200px] max-h-[150px] rounded-lg border border-slate-200" />
                    ...
                    <p class="text-xs text-slate-400 mt-1">Upload new to replace</p>
                ...
                <div>
                    <x-base.form-label>Back of License</x-base.form-label>
                    @if($license->getFirstMediaUrl('license_back'))
                    <div class="mb-3">
                        <img src="{{ $license->getFirstMediaUrl('license_back') }}" 
                             alt="Current back" class="max-w-[200px] max-h-[150px] rounded-lg border border-slate-200" />
                        <p class="text-xs text-slate-500 mt-1">Current image</p>
                    </div>
                    @endif
        ...
        <div class="box box--stacked p-6">
            ...
            @php
                $existingDocs = $license->getMedia('license_documents');
            @endphp
            
            @if($existingDocs->count() > 0)
            <div class="mb-4">
                <p class="text-sm font-medium text-slate-700 mb-2">Current Documents:</p>
                <div class="space-y-2">
                    ...
                    <div class="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                        <div class="flex items-center gap-2">
                            ...
                            @endif
                            <a href="{{ $doc->getUrl() }}" target="_blank" class="text-sm text-primary hover:underline">{{ $doc->file_name }}</a>
                            <span class="text-xs text-slate-400">({{ number_format($doc->size / 1024, 1) }} KB)</span>
                        </div>
                        <button type="button" onclick="if(confirm('Delete this document?')) document.getElementById('delete-doc-{{ $doc->id }}').submit();" 
            ...
            <div>
                <x-base.form-label for="documents">Add New Documents</x-base.form-label>
        ...
        <div class="flex justify-end gap-3">
            <x-base.button as="a" href="{{ route('driver.licenses.show', $license) }}" variant="outline-secondary">
                Cancel
            </x-base.button>
...
@foreach($license->getMedia('license_documents') as $doc)
<form id="delete-doc-{{ $doc->id }}" action="{{ route('driver.licenses.delete-document', ['license' => $license->id, 'media' => $doc->id]) }}" method="POST" class="hidden">
    @csrf
    @method('DELETE')
</form>
@endforeach
