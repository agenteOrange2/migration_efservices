...
@section('title', 'Driver Accidents')
@php
$breadcrumbLinks = [
    ['label' => 'App', 'url' => route('carrier.dashboard')],
    ['label' => 'Driver Accidents', 'active' => true],
];
@endphp
...
<div>
        ...
        <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
            <div class="flex flex-col w-full md:flex-row text-center md:text-left items-center gap-4">
                <div class="p-3 bg-primary/10 rounded-xl border border-primary/20">
                    <x-base.lucide class="w-8 h-8 text-primary" icon="AlertTriangle" />
                </div>
                <div>
                    <h1 class="text-3xl font-bold text-slate-800 mb-2">Driver Accidents Management</h1>
                    <p class="text-slate-600">Manage and track driver accident records</p>
                </div>
            </div>
            <div class="flex flex-col justify-center md:flex-row md:justify-end gap-3 w-full">
                <x-base.button as="a" href="{{ route('carrier.drivers.accidents.documents.index') }}" class="w-full sm:w-auto" variant="outline-primary">
                    <x-base.lucide class="w-4 h-4 mr-2" icon="file-text" />
                    View All Documents
                </x-base.button>
                <x-base.button as="a" href="{{ route('carrier.drivers.accidents.create') }}" class="w-full sm:w-auto" variant="primary">
    ...
    <div class="box box--stacked mt-5 p-3">
        ...
        <div class="box-body p-5">
            <form action="{{ route('carrier.drivers.accidents.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
    ...
    <div class="box box--stacked mt-5 p-3">
        <div class="box-header">
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                <h3 class="box-title">Accidents ({{ $accidents->total() ?? 0 }})</h3>
            </div>
        </div>
                    ...
                    <x-base.table.thead>
                        <x-base.table.tr>
                            <x-base.table.th class="whitespace-nowrap">
                                <a href="{{ route('carrier.drivers.accidents.index', array_merge(request()->except(['sort_field', 'sort_direction']), ['sort_field' => 'created_at', 'sort_direction' => request('sort_field') == 'created_at' && request('sort_direction') == 'asc' ? 'desc' : 'asc'])) }}" class="flex items-center">
                                    Registration Date
                            ...
                            <x-base.table.th class="whitespace-nowrap">
                                <a href="{{ route('carrier.drivers.accidents.index', array_merge(request()->except(['sort_field', 'sort_direction']), ['sort_field' => 'accident_date', 'sort_direction' => request('sort_field') == 'accident_date' && request('sort_direction') == 'asc' ? 'desc' : 'asc'])) }}" class="flex items-center">
                                    Accident Date
                                ...
                                <x-base.menu class="h-5">
                                    ...
                                    <x-base.menu.items class="w-48">
                                        <div class="flex flex-col gap-3">
                                            <a href="{{ route('carrier.drivers.accidents.driver_history', $accident->userDriverDetail->id) }}" class="flex mr-1 text-primary" title="View Driver History">
                                                <x-base.lucide class="w-4 h-4 mr-3" icon="history" />
                                                Driver History
                                            </a>
