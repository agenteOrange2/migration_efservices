...
@section('subcontent')
        ...
        <div class="col-span-12">
            ...
            <div class="flex flex-col md:flex-row items-center justify-between">
                ...
                <div class="flex flex-wrap gap-2 mt-4 md:mt-0">
                    <form action="{{ route('carrier.maintenance.generate-report', $maintenance->id) }}" method="POST" class="inline">
                        @csrf
                    ...
                    @endif
                    <form action="{{ route('carrier.maintenance.toggle-status', $maintenance->id) }}" method="POST" class="inline">
                        @csrf
                        <x-base.button type="submit" variant="{{ $maintenance->status ? 'outline-secondary' : 'outline-success' }}" 
                            class="w-full sm:w-auto">
                            <x-base.lucide class="w-4 h-4 mr-2" icon="{{ $maintenance->status ? 'XCircle' : 'CheckCircle' }}" />
                            {{ $maintenance->status ? 'Mark as Pending' : 'Mark as Completed' }}
                        </x-base.button>
                    </form>
                    <x-base.button as="a" href="{{ route('carrier.maintenance.edit', $maintenance->id) }}" 
                ...
                <div class="p-5">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="bg-slate-50/50 p-3 rounded-lg">
                                    ...
                                    <div class="font-medium">{{ $maintenance->vehicle->make }} {{ $maintenance->vehicle->model }}</div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div class="bg-slate-50/50 p-3 rounded-lg">
                                    ...
                                    <div class="font-medium">{{ $maintenance->service_date ? $maintenance->service_date->format('m/d/Y') : 'Not established' }}</div>
                                </div>
                                
                                @if($maintenance->next_service_date)
                                    <div class="bg-slate-50/50 p-3 rounded-lg col-span-full {{ $maintenance->isOverdue() ? 'bg-danger/10' : ($maintenance->isUpcoming() ? 'bg-warning/10' : '') }}">
                                        <div class="text-sm text-slate-500">Next Service Date</div>
                                        <div class="font-medium">{{ $maintenance->next_service_date->format('m/d/Y') }}</div>
                                        @if(!$maintenance->status)
                                            @if($maintenance->isOverdue())
                                                <div class="text-xs text-danger mt-1">
                                                    <x-base.lucide class="w-3 h-3 inline mr-1" icon="AlertTriangle" />
                                                    Overdue by {{ floor($maintenance->next_service_date->diffInDays(now())) }} days
                                                </div>
                                            @elseif($maintenance->isUpcoming())
                                                <div class="text-xs text-warning mt-1">
                                                    <x-base.lucide class="w-3 h-3 inline mr-1" icon="Clock" />
                                                    Due in {{ floor(now()->diffInDays($maintenance->next_service_date)) }} days
                                                </div>
                                            @else
                    ...
                    <div class="mt-8">
                        ...
                        <div class="mt-5 p-4 bg-slate-50/60 rounded-lg border border-dashed border-slate-300">
                            <form action="{{ route('carrier.maintenance.store-documents', $maintenance->id) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                            ...
                                            <tr class="bg-white border-b hover:bg-gray-50">
                                                ...
                                                <td class="px-4 py-3 text-center">
                                                    <div class="flex justify-center gap-2">
                                                        ...
                                                        @endif
                                                        <form action="{{ route('carrier.maintenance.delete-report', [$maintenance->id, $report->id]) }}" method="POST" class="inline"
    ...
    <x-base.dialog id="reschedule-modal" size="md">
        <x-base.dialog.panel>
            <form id="reschedule-form" action="{{ route('carrier.maintenance.reschedule', $maintenance->id) }}" method="POST">
                @csrf
                <div class="p-5">
                    ...
                    <div class="mt-5 grid grid-cols-12 gap-4">
                        ...
                        <div class="col-span-12">
                            ...
                            <textarea id="reschedule_reason" name="reschedule_reason" 
                                ...
                                rows="4"
                                placeholder="Explain why this maintenance is being rescheduled..." 
                                minlength="3"
                                required></textarea>
...
<script>
    ...
    window.deleteDocument = function(documentId) {
        ...
        fetch(`/carrier/maintenance/documents/${documentId}`, {
            method: 'DELETE',
            headers: {
