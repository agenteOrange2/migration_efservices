    ...
    <div class="col-span-12">
        <div class="flex flex-col gap-y-3 md:h-10 md:flex-row md:items-center md:justify-between">
            ...
            <div class="flex gap-2">
                <a href="{{ route('carrier.reports.repairs.export-pdf', request()->query()) }}" 
        ...
        <div class="mt-3.5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-6 gap-4 mb-5">
                <div class="box box--stacked p-4">
                    <div class="text-xs text-slate-500 uppercase mb-1">Total Repairs</div>
                ...
                <div class="box box--stacked p-4">
                    ...
                    <div class="text-2xl font-medium text-info">{{ $stats['in_progress'] ?? 0 }}</div>
                </div>
                <div class="box box--stacked p-4">
            ...
            <div class="box box--stacked p-5 mb-5">
                ...
                <form action="{{ route('carrier.reports.repairs') }}" method="GET" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" 
                               ...
                               placeholder="Repair name, description, unit..."
                    ...
                    <div class="col-span-full flex gap-2">
                        ...
                        <a href="{{ route('carrier.reports.repairs') }}" class="transition duration-200 border shadow-sm inline-flex items-center justify-center py-2 px-3 rounded-md font-medium cursor-pointer focus:ring-4 focus:ring-primary focus:ring-opacity-20 focus-visible:outline-none dark:focus:ring-slate-700 dark:focus:ring-opacity-50 [&:hover:not(:disabled)]:bg-opacity-90 [&:hover:not(:disabled)]:border-opacity-90 [&:not(button)]:text-center disabled:opacity-70 disabled:cursor-not-allowed bg-secondary/70 border-secondary/70 text-slate-500 dark:border-darkmode-400 dark:bg-darkmode-400 dark:text-slate-300 [&:hover:not(:disabled)]:bg-slate-100 [&:hover:not(:disabled)]:border-slate-100">
            ...
            <div class="box box--stacked">
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-b border-slate-200/60">
                        ...
                        <tbody>
                            @forelse($repairRecords as $repair)
                                <tr class="border-b border-slate-200/60">
                                    <td class="px-5 py-4 border-slate-200/60">
                                        <div>
                                            <div class="font-medium">{{ $repair->vehicle->company_unit_number ?? 'N/A' }}</div>
                                            <div class="text-xs text-slate-500">{{ $repair->vehicle->make ?? '' }} {{ $repair->vehicle->model ?? '' }}</div>
                                        </div>
                                    </td>
                                    <td class="px-5 py-4 border-slate-200/60">
                                        <div class="font-medium">{{ $repair->repair_name ?? 'N/A' }}</div>
                                    ...
                                    <td class="px-5 py-4 border-slate-200/60">
                                        <span class="font-medium">${{ number_format($repair->cost ?? 0, 2) }}</span>
                                    </td>
                                    <td class="px-5 py-4 border-slate-200/60">
                                        ...
                                        @elseif($repair->status === 'in_progress')
                                            <span class="inline-flex items-center px-2 py-1 text-xs font-medium rounded-full bg-info/10 text-info">
                                                In Progress
                                            </span>
                                        @else
                                ...
                                <tr>
                                    <td colspan="5" class="px-5 py-8 text-center text-slate-500">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <div class="text-base font-medium">No repair records found</div>
                ...
                @if($repairRecords->hasPages())
                    <div class="flex flex-col md:flex-row items-center justify-between p-5 border-t border-slate-200/60">
                        <div class="text-sm text-slate-500 mb-3 md:mb-0">
                            Showing {{ $repairRecords->firstItem() }} to {{ $repairRecords->lastItem() }} of {{ $repairRecords->total() }} records
                        </div>
                        <div>
                            {{ $repairRecords->appends($filters)->links() }}
                        </div>
                    </div>
                @endif
            </div>
