    ...
    <div class="col-span-12">
        <div class="flex flex-col gap-y-3 md:h-10 md:flex-row md:items-center md:justify-between">
            ...
            <div class="flex gap-2">
                <a href="{{ route('carrier.reports.accidents.export-pdf', request()->query()) }}" 
        ...
        <div class="mt-3.5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-5">
                <div class="box box--stacked p-4">
                    <div class="text-xs text-slate-500 uppercase mb-1">Total Accidents</div>
                    <div class="text-2xl font-medium">{{ $stats['total'] ?? 0 }}</div>
                </div>
                <div class="box box--stacked p-4">
                    <div class="text-xs text-slate-500 uppercase mb-1">Recent (30 days)</div>
                    <div class="text-2xl font-medium text-primary">{{ $stats['recent'] ?? 0 }}</div>
                </div>
                <div class="box box--stacked p-4">
                    <div class="text-xs text-slate-500 uppercase mb-1">With Fatalities</div>
                    <div class="text-2xl font-medium text-danger">{{ $stats['with_fatalities'] ?? 0 }}</div>
                </div>
                <div class="box box--stacked p-4">
                    <div class="text-xs text-slate-500 uppercase mb-1">With Injuries</div>
                    <div class="text-2xl font-medium text-warning">{{ $stats['with_injuries'] ?? 0 }}</div>
                </div>
                <div class="box box--stacked p-4">
                    <div class="text-xs text-slate-500 uppercase mb-1">Without Injuries</div>
                    <div class="text-2xl font-medium text-success">{{ $stats['without_injuries'] ?? 0 }}</div>
                </div>
            </div>
            ...
            <div class="box box--stacked p-5 mb-5">
                ...
                <form action="{{ route('carrier.reports.accidents') }}" method="GET" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div class="col-span-full flex gap-2">
                        ...
                        <a href="{{ route('carrier.reports.accidents') }}" class="transition duration-200 border shadow-sm inline-flex items-center justify-center py-2 px-3 rounded-md font-medium cursor-pointer focus:ring-4 focus:ring-primary focus:ring-opacity-20 focus-visible:outline-none dark:focus:ring-slate-700 dark:focus:ring-opacity-50 [&:hover:not(:disabled)]:bg-opacity-90 [&:hover:not(:disabled)]:border-opacity-90 [&:not(button)]:text-center disabled:opacity-70 disabled:cursor-not-allowed bg-secondary/70 border-secondary/70 text-slate-500 dark:border-darkmode-400 dark:bg-darkmode-400 dark:text-slate-300 [&:hover:not(:disabled)]:bg-slate-100 [&:hover:not(:disabled)]:border-slate-100">
                    ...
                    <table class="w-full text-left border-b border-slate-200/60">
                        ...
                        <tbody>
                            @forelse($accidents as $accident)
                                <tr class="border-b border-slate-200/60 {{ $accident->had_fatalities ? 'bg-danger/5' : ($accident->had_injuries ? 'bg-warning/5' : '') }}">
                                    <td class="px-5 py-4 border-slate-200/60">
                                        <div class="font-medium">{{ $accident->accident_date ? $accident->accident_date->format('m/d/Y') : 'N/A' }}</div>
                                    </td>
                                    <td class="px-5 py-4 border-slate-200/60">
                                        @if($accident->userDriverDetail)
                                            <div class="font-medium">{{ $accident->userDriverDetail->full_name }}</div>
                                            @if($accident->userDriverDetail->user)
                                                <div class="text-xs text-slate-500">{{ $accident->userDriverDetail->user->email }}</div>
                                            @endif
                                        @else
                                    ...
                                    <td class="px-5 py-4 border-slate-200/60">
                                        <div class="max-w-xs truncate">{{ $accident->nature_of_accident ?? 'N/A' }}</div>
                                    </td>
                                    <td class="px-5 py-4 border-slate-200/60">
                                        ...
                                        @else
                                            <span class="inline-flex items-center px-2 py-1 text-xs font-medium rounded-full bg-slate-100 text-slate-600">
                                                <x-base.lucide class="mr-1 h-3 w-3" icon="Info" />
                                                Minor
                                            </span>
                                        @endif
                                    </td>
                                    <td class="px-5 py-4 border-slate-200/60">
                                        @if($accident->had_fatalities)
                                            <span class="text-danger font-medium">{{ $accident->number_of_fatalities ?? 0 }}</span>
                                        @else
                                    ...
                                    <td class="px-5 py-4 border-slate-200/60">
                                        @if($accident->had_injuries)
                                            <span class="text-warning font-medium">{{ $accident->number_of_injuries ?? 0 }}</span>
                                        @else
                                            <span class="text-slate-400">0</span>
                                        @endif
                                    </td>
                                    <td class="px-5 py-4 border-slate-200/60">
                                        @if($accident->comments)
                                            <div class="max-w-xs truncate text-sm text-slate-600" title="{{ $accident->comments }}">
                                                {{ $accident->comments }}
                                            </div>
                                        @else
                                ...
                                <tr>
                                    <td colspan="7" class="px-5 py-8 text-center text-slate-500">
                                        <div class="flex flex-col items-center">
                                            <x-base.lucide class="h-12 w-12 text-slate-300 mb-3" icon="AlertCircle" />
                                            <div class="text-base font-medium">No accidents found</div>
                ...
                @if($accidents->hasPages())
                    <div class="flex flex-col md:flex-row items-center justify-between p-5 border-t border-slate-200/60">
                        <div class="text-sm text-slate-500 mb-3 md:mb-0">
                            Showing {{ $accidents->firstItem() }} to {{ $accidents->lastItem() }} of {{ $accidents->total() }} accidents
                        </div>
                        <div>
                            {{ $accidents->appends($filters)->links() }}
                        </div>
                    </div>
                @endif
            </div>
