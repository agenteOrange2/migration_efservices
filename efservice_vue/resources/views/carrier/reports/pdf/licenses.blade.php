...
<body>
    ...
    <div class="carrier-info">
        ...
        <strong>Total Records:</strong> {{ $total_licenses }}
    </div>
    ...
    @if(!empty($filters) && array_filter(array_diff_key($filters, ['per_page' => ''])))
        <div class="filters">
            ...
            @if(!empty($filters['license_type'])) Type: {{ ucfirst($filters['license_type']) }}. @endif
    ...
    <table>
        ...
        <tbody>
            ...
            @php
                ...
                $daysLeft = $license->expiration_date ? (int) now()->diffInDays($license->expiration_date, false) : null;
                $isExpiring = !$isExpired && $daysLeft !== null && $daysLeft >= 0 && $daysLeft <= 30;
            @endphp
            <tr class="{{ $isExpired ? 'expired' : ($isExpiring ? 'expiring' : '') }}">
                <td>
                    <strong>{{ $license->driverDetail?->full_name ?? 'N/A' }}</strong>
                ...
                <td><strong>{{ $license->license_number ?? 'N/A' }}</strong></td>
                <td>{{ $license->state_of_issue ?? 'N/A' }}</td>
                <td>
                    {{ $license->is_cdl ? 'CDL' : 'Non-CDL' }}
                    @if($license->license_class) — Class {{ $license->license_class }} @endif
                </td>
                <td>{{ $license->created_at ? $license->created_at->format('m/d/Y') : 'N/A' }}</td>
                <td>
                    @if($license->expiration_date)
                        <strong>{{ $license->expiration_date->format('m/d/Y') }}</strong>
                    @else
                        N/A
                    @endif
                </td>
            ...
            @empty
            <tr>
                <td colspan="7" style="text-align: center; padding: 20px; color: #94a3b8;">No licenses found matching the specified filters</td>
            </tr>
            @endforelse
        </tbody>
    </table>
    ...
    <div class="footer">
        <p>{{ $carrier->name }} - License Report | &copy; {{ now()->format('Y') }} EFCTS. All rights reserved. | Confidential</p>
    </div>
</body>
</html>
