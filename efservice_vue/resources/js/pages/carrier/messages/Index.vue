...
function applyFilters() {
    router.get(route('carrier.messages.index'), {
...
function deleteMessage(message: MessageRow) {
    ...
    router.delete(route('carrier.messages.destroy', message.id), { preserveScroll: true })
}

function duplicateMessage(message: MessageRow) {
    if (!confirm(`Create a duplicate of "${message.subject}" as a new draft?`)) return
    router.post(route('carrier.messages.duplicate', message.id), {}, { preserveScroll: true })
}

function resendMessage(message: MessageRow) {
    if (!confirm(`Resend "${message.subject}" to all recipients?`)) return
    router.post(route('carrier.messages.resend', message.id), {}, { preserveScroll: true })
}
</script>

<template>
    <Head title="Carrier Messages" />
    ...
    <div class="grid grid-cols-12 gap-6">
                ...
                <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-slate-500">Review sent and received conversations for your carrier, then draft or resend messages when needed.</p>
                        </div>
                    </div>
        ...
        <div class="col-span-12 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-5">
            <div class="box box--stacked p-5">
                <p class="text-sm text-slate-500">Accessible</p>
