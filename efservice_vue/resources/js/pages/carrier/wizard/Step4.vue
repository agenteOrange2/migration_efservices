...
import type { CarrierBankingDetail } from '@/types'

interface Props {
    bankingDetails: CarrierBankingDetail | null
}
...
function submit() {
    form.post(route('carrier.wizard.step4.process'), { preserveScroll: true })
}
</script>

<template>
    <Head title="Carrier Registration - Step 4" />
            ...
            <div class="relative z-10 flex flex-col justify-center w-full h-full py-2 lg:py-16">
                <div class="rounded-[0.8rem] w-[55px] h-[55px] border border-primary/30 flex items-center justify-center">
                    <div class="relative flex items-center justify-center w-[50px] rounded-[0.6rem] h-[50px] bg-linear-to-b from-theme-1/90 to-theme-2/90 bg-white">
                        <Lucide icon="Landmark" class="w-8 h-8 text-primary" />
                    </div>
                </div>
                ...
                <div class="mt-6">
                    <div class="text-2xl font-medium">Banking Information</div>
                    <div class="mt-2.5 text-slate-600">Required for payment processing</div>

                    <div class="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg flex items-start gap-2.5">
                        <Lucide icon="ShieldCheck" class="w-5 h-5 text-green-600 mt-0.5 shrink-0" />
                        <div>
                            <div class="text-sm font-medium text-green-800">Your data is secure</div>
                            <div class="text-xs text-green-700 mt-0.5">All banking information is encrypted and PCI DSS compliant.</div>
                        </div>
                    </div>
                    ...
                    <form @submit.prevent="submit" class="mt-5 space-y-4">
                        <div>
                            ...
                            <FormInput v-model="form.account_holder_name" type="text" required class="block px-4 py-3.5 rounded-[0.6rem] border-slate-300/80" placeholder="John Doe" />
                            <p v-if="form.errors.account_holder_name" class="mt-1 text-xs text-red-500">{{ form.errors.account_holder_name }}</p>
                        </div>
                        <div>
                            <FormLabel>Account Number *</FormLabel>
                            <div class="relative">
                                <div class="absolute left-3 top-1/2 -translate-y-1/2"><Lucide icon="Lock" class="w-4 h-4 text-slate-400" /></div>
                                <FormInput v-model="form.account_number" type="text" required class="block pl-10 pr-4 py-3.5 rounded-[0.6rem] border-slate-300/80" placeholder="••••••••••" />
                            </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <div class="absolute left-3 top-1/2 -translate-y-1/2"><Lucide icon="Lock" class="w-4 h-4 text-slate-400" /></div>
                                <FormInput v-model="form.banking_routing_number" type="text" required class="block pl-10 pr-4 py-3.5 rounded-[0.6rem] border-slate-300/80" placeholder="•••••••••" maxlength="9" />
                            </div>
                            <p v-if="form.errors.banking_routing_number" class="mt-1 text-xs text-red-500">{{ form.errors.banking_routing_number }}</p>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <div class="absolute left-3 top-1/2 -translate-y-1/2"><Lucide icon="Lock" class="w-4 h-4 text-slate-400" /></div>
                                    <FormInput v-model="form.security_code" type="password" required class="block pl-10 pr-4 py-3.5 rounded-[0.6rem] border-slate-300/80" placeholder="•••" />
                                </div>
                                <p v-if="form.errors.security_code" class="mt-1 text-xs text-red-500">{{ form.errors.security_code }}</p>
                            </div>
                        </div>
    ...
    <div class="fixed container grid w-screen inset-0 h-screen grid-cols-12 lg:max-w-[1550px] 2xl:max-w-[1750px] pl-14 pr-12 xl:px-24">
        ...
        <div :class="['h-full col-span-7 2xl:col-span-8 lg:relative', 'before:content-[\'\'] before:absolute before:lg:-ml-10 before:left-0 before:inset-y-0 before:bg-linear-to-b before:from-theme-1 before:to-theme-2 before:w-screen before:lg:w-[800%]', 'after:content-[\'\'] after:absolute after:inset-y-0 after:left-0 after:w-screen after:lg:w-[800%] after:bg-texture-white after:bg-fixed after:bg-center after:lg:bg-[25rem_-25rem] after:bg-no-repeat']">
            <div class="sticky top-0 z-10 flex-col justify-center hidden h-screen ml-16 lg:flex xl:ml-28 2xl:ml-36">
                <div class="leading-[1.4] text-[2.6rem] xl:text-5xl font-medium xl:leading-[1.2] text-white">Banking<br />Details</div>
                <div class="mt-5 text-base leading-relaxed xl:text-lg text-white/70">Secure payment information to finalize your carrier registration.</div>
