...
const props = defineProps({
    ...
    maintenanceRecords: { type: Object, required: true },
    vehicles: { type: Array, default: () => [] },
    statusOptions: { type: Array, default: () => [] },
})
const { filters: initialFilters, stats, maintenanceRecords, vehicles, statusOptions } = props
const filters = reactive({ ...initialFilters })
const statCards = [
    { label: 'Total Records', value: stats?.count ?? 0, icon: 'Wrench' },
    { label: 'Completed', value: stats?.completed ?? 0, icon: 'BadgeCheck' },
    { label: 'Pending', value: stats?.pending ?? 0, icon: 'Clock3' },
    { label: 'Average Cost', value: money(stats?.average_cost ?? 0), icon: 'BadgeDollarSign' },
]

function applyFilters() {
    router.get(route('carrier.reports.maintenance'), { ...filters }, { preserveState: true, preserveScroll: true, replace: true })
}
...
<template>
    ...
    <div class="grid grid-cols-12 gap-6">
        <div class="col-span-12">
            <ReportHero title="Maintenance Reports" subtitle="Scheduled maintenance cost, vendor, and completion tracking." icon="Wrench">
                <template #actions>
                    <Button as="a" :href="route('carrier.reports.maintenance.export-pdf', { ...filters })" variant="outline-secondary" class="gap-2">
        ...
        <div class="col-span-12">
            <div class="box box--stacked p-5">
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-6">
                    <FormInput v-model="filters.search" type="text" placeholder="Search service task, vendor..." @keyup.enter="applyFilters" />
                    ...
                    <FormInput v-model="filters.type" type="text" placeholder="Service type..." @keyup.enter="applyFilters" />
                    ...
                    <table class="min-w-full divide-y divide-slate-200 text-sm">
                        <thead class="bg-slate-50">
                            <tr>
                                ...
                                <th class="px-5 py-3 text-left font-semibold text-slate-600">Service Task</th>
                                <th class="px-5 py-3 text-left font-semibold text-slate-600">Service Date</th>
                                <th class="px-5 py-3 text-left font-semibold text-slate-600">Next Service</th>
                                <th class="px-5 py-3 text-left font-semibold text-slate-600">Vendor</th>
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white">
                            <tr v-for="row in maintenanceRecords.data" :key="row.id">
                                <td class="px-5 py-4 font-medium text-slate-800">{{ row.vehicle_label || 'N/A' }}</td>
                                <td class="px-5 py-4 text-slate-600">
                                    <div>{{ row.service_tasks || 'N/A' }}</div>
                                    <div class="text-xs text-slate-500">{{ row.description || '-' }}</div>
                                </td>
                                <td class="px-5 py-4 text-slate-600">{{ row.service_date || 'N/A' }}</td>
                                <td class="px-5 py-4 text-slate-600">{{ row.next_service_date || 'N/A' }}</td>
                                <td class="px-5 py-4 text-slate-600">{{ row.vendor_mechanic || 'N/A' }}</td>
                            ...
                            <tr v-if="!maintenanceRecords.data.length"><td colspan="7" class="px-5 py-10 text-center text-slate-500">No maintenance records matched the current filters.</td></tr>
                        </tbody>
                    </table>
                </div>
                <div class="border-t border-slate-100 p-4"><ReportPagination :links="maintenanceRecords.links" /></div>
            </div>
        </div>
    </div>
</template>
