...
<template>
    <Head title="Carrier HOS Dashboard" />
    ...
    <div class="grid grid-cols-12 gap-6">
            ...
            <div class="box box--stacked p-6">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h1 class="text-2xl font-bold text-slate-800">HOS Dashboard</h1>
                            <p class="mt-1 text-sm text-slate-500">{{ carrier.name }} hours-of-service overview with live driver status and compliance signals.</p>
                        </div>
                    </div>

                    <div class="flex flex-wrap gap-3">
                        <Button as="a" :href="route('carrier.hos.documents.index')" variant="outline-secondary" class="gap-2">
                            <Lucide icon="FileText" class="h-4 w-4" />
                            Documents
                        </Button>
                        <Button as="a" :href="route('carrier.hos.fmcsa.configuration')" variant="outline-secondary" class="gap-2">
                            <Lucide icon="Settings" class="h-4 w-4" />
                            Configuration
                        </Button>
                        <Button as="a" :href="route('carrier.violations.index')" variant="primary" class="gap-2">
        ...
        <div class="col-span-12 xl:col-span-4">
            <div class="box box--stacked p-5">
                <h2 class="text-lg font-semibold text-slate-800">Current FMCSA Rules</h2>
                <div class="mt-4 space-y-3 text-sm">
                    ...
                    <div class="flex items-center justify-between"><span class="text-slate-500">Warning threshold</span><span class="font-medium text-slate-800">{{ config.warning_threshold_minutes }} min</span></div>
                    <div class="flex items-center justify-between"><span class="text-slate-500">Violation threshold</span><span class="font-medium text-slate-800">{{ config.violation_threshold_minutes }} min</span></div>
                    <div class="flex items-center justify-between"><span class="text-slate-500">60 / 7 weekly limit</span><span class="font-medium text-slate-800">{{ Math.round(config.weekly_limit_60_minutes / 60) }} h</span></div>
                    <div class="flex items-center justify-between"><span class="text-slate-500">70 / 8 weekly limit</span><span class="font-medium text-slate-800">{{ Math.round(config.weekly_limit_70_minutes / 60) }} h</span></div>
                    <div class="flex items-center justify-between"><span class="text-slate-500">30-minute break</span><span class="font-medium text-slate-800">{{ config.require_30_min_break ? `Required after ${config.break_after_hours}h` : 'Disabled' }}</span></div>
                    <div class="flex items-center justify-between"><span class="text-slate-500">Texas intrastate mode</span><span class="font-medium text-slate-800">{{ config.fmcsa_texas_mode ? 'Enabled' : 'Disabled' }}</span></div>
                    <div class="flex items-center justify-between"><span class="text-slate-500">24-hour reset</span><span class="font-medium text-slate-800">{{ config.allow_24_hour_reset ? 'Allowed' : 'Disabled' }}</span></div>
                    <div class="flex items-center justify-between"><span class="text-slate-500">Ghost log detection</span><span class="font-medium text-slate-800">{{ config.enable_ghost_log_detection ? `${config.ghost_log_threshold_minutes} min` : 'Disabled' }}</span></div>
                </div>
            </div>
        </div>
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white">
                            <tr v-for="driver in driverSummaries" :key="driver.id">
                                ...
                                <td class="px-5 py-4 text-right">
                                    <Link :href="route('carrier.hos.driver.log', driver.id)" class="inline-flex items-center gap-2 text-primary hover:underline">
