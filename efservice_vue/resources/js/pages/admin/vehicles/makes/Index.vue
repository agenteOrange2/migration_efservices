...
const props = defineProps<{
    makes: { data: any[]; links: { url: string | null; label: string; active: boolean }[]; total: number; last_page: number }
...
const defaultRouteNames = {
    index: 'admin.vehicle-makes.index',
    store: 'admin.vehicle-makes.store',
    update: 'admin.vehicle-makes.update',
    destroy: 'admin.vehicle-makes.destroy',
} as const
...
function deleteMake(make: any) {
    if (!confirm(`Delete "${make.name}"?`)) return
    router.delete(namedRoute('destroy', make.id), { preserveScroll: true })
}
</script>
...
<template>
    ...
    <div class="p-5 sm:p-8 max-w-screen-2xl mx-auto">
            ...
            <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
                <div class="flex items-center gap-4">
                    <div class="p-3 bg-primary/10 rounded-xl border border-primary/20">
                        <Lucide icon="Tag" class="w-8 h-8 text-primary" />
                    </div>
                    <div>
                        <h1 class="text-3xl font-bold text-slate-800 mb-1">Vehicle Makes</h1>
                        <p class="text-slate-500 text-sm">Manage available vehicle makes across the fleet module.</p>
                    </div>
                </div>
        ...
        <div class="box box--stacked p-6 mb-6">
            <div class="relative max-w-xl">
                ...
                <input v-model="search" type="text" placeholder="Search vehicle make..." class="w-full text-sm border-slate-200 shadow-sm rounded-lg py-2.5 px-3 pl-10 border focus:ring-primary focus:border-primary" />
            </div>
        </div>
        ...
        <div class="box box--stacked">
            <div class="p-6 border-b border-slate-200/60 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-lg font-semibold text-slate-800">Vehicle Make List</h2>
                </div>
                <span class="bg-primary/10 text-primary text-xs font-semibold px-3 py-1.5 rounded-full">{{ makes.total }} Total</span>
            </div>
                ...
                <table class="w-full">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-200/60">
                            <th class="px-6 py-4 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">Make</th>
                    ...
                    <tbody class="divide-y divide-slate-200/60">
                        <tr v-for="make in makes.data" :key="make.id" class="hover:bg-slate-50/50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="p-2 bg-primary/10 rounded-lg"><Lucide icon="Tag" class="w-4 h-4 text-primary" /></div>
                                    <div class="font-medium text-slate-800">{{ make.name }}</div>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-sm text-slate-600">{{ make.created_at }}</td>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    <button @click="openEdit(make)" class="p-1.5 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-100 transition-colors"><Lucide icon="Edit" class="w-4 h-4" /></button>
                                    <button @click="deleteMake(make)" :disabled="make.vehicles_count > 0" class="p-1.5 rounded-lg border border-danger/20 text-danger hover:bg-danger/10 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"><Lucide icon="Trash2" class="w-4 h-4" /></button>
                                </div>
                            </td>
                        </tr>
                        <tr v-if="!makes.data.length">
                            <td colspan="4" class="px-6 py-12 text-center">
                                <Lucide icon="Tag" class="w-12 h-12 text-slate-300 mx-auto mb-3" />
                                <p class="text-slate-500 font-medium">No vehicle makes found</p>
                            </td>
                        </tr>
                    </tbody>
                </table>
            ...
            <div v-if="makes.last_page > 1" class="p-6 border-t border-slate-200/60 flex flex-wrap items-center gap-1">
    ...
    <Dialog :open="showModal" @close="showModal = false" static-backdrop>
        <Dialog.Panel class="w-full max-w-xl overflow-hidden">
            <div class="sticky top-0 bg-white px-6 pt-6 pb-4 border-b border-slate-200 z-10">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="text-lg font-bold text-slate-800">{{ modalMode === 'create' ? 'Add Vehicle Make' : 'Edit Vehicle Make' }}</h3>
                    </div>
            ...
            <div class="px-6 py-5">
                <label class="block text-sm font-medium text-slate-700 mb-1">Make Name <span class="text-red-500">*</span></label>
                <input v-model="form.name" type="text" class="w-full border-slate-300 rounded-lg text-sm px-3 py-2.5 border focus:ring-primary focus:border-primary" placeholder="Enter make name" />
            ...
            <div class="sticky bottom-0 bg-white px-6 py-4 border-t border-slate-200 flex justify-end gap-3">
                ...
                <button @click="saveForm" :disabled="saving" class="px-4 py-2 text-sm rounded-lg bg-primary text-white hover:bg-primary/90 transition-colors font-medium disabled:opacity-60 flex items-center gap-2">
                    ...
                    {{ saving ? 'Saving…' : (modalMode === 'create' ? 'Create Make' : 'Save Changes') }}
                </button>
            </div>
        </Dialog.Panel>
    </Dialog>
