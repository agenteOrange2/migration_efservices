...
const statCards = computed(() => [{ label: 'Total Logs', value: props.stats.total_logs, icon: 'Clock', tone: 'primary' }, { label: 'Logs with Violations', value: props.stats.logs_with_violations, icon: 'AlertTriangle', tone: 'danger' }, { label: 'Compliance %', value: `${props.stats.compliance_percentage}%`, icon: 'ShieldCheck', tone: 'success' }, { label: 'Avg Driving Hrs', value: props.stats.average_driving_hours, icon: 'Gauge', tone: 'info' }])
function applyFilters() { router.get(route('admin.reports.hos'), { carrier_id: filters.carrier_id || undefined, driver_id: filters.driver_id || undefined, date_from: filters.date_from || undefined, date_to: filters.date_to || undefined, has_violations: filters.has_violations || undefined }, { preserveScroll: true, replace: true }) }
</script>
<template>
