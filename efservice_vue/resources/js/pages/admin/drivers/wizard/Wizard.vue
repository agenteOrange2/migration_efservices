...
const fileInputClass =
    'w-full text-sm text-slate-500 cursor-pointer file:mr-3 file:cursor-pointer file:rounded-md file:border-0 file:bg-primary/10 file:px-3 file:py-1.5 file:text-xs file:font-medium file:text-primary hover:file:bg-primary/20';
...
const props = withDefaults(
    ...
    }>(),
    {
        ...
        routeNames: () => ({
            ...
            employmentMarkEmailStatus:
                'admin.drivers.employment.mark-email-status',
        }),
    },
);
...
watch(
    () => props.initialStep,
    (newStep) => {
        if (newStep != null) currentStep.value = newStep;
    },
);
const totalSteps = 15;
...
function handlePhotoChange(e: Event) {
    ...
    if (file) {
        ...
        reader.onload = (ev) => {
            step1.photoPreview = ev.target?.result as string;
        };
...
function submitStep1() {
    ...
    if (!isEditMode.value && !step1.password)
        errs.push('Password is required.');
    if (!step1.terms_accepted)
    ...
    if (isEditMode.value) {
        ...
        router.post(
            namedRoute('updateStep', { driver: props.driver!.id, step: 1 }),
            data,
        );
    } else {
...
const step2 = reactive({
    ...
    lived_three_years:
        props.stepData?.step2?.primary?.lived_three_years ?? false,
    previous_addresses: ((props.stepData?.step2?.previous ?? []) as any[]).map(
        (a: any) => ({
            ...a,
...
function submitStep2() {
    ...
    for (const [i, a] of step2.previous_addresses.entries()) {
        if (!a.address_line1?.trim())
            errs.push(`Previous address #${i + 1}: Address is required.`);
        if (!a.city?.trim())
            errs.push(`Previous address #${i + 1}: City is required.`);
        if (!a.state)
            errs.push(`Previous address #${i + 1}: State is required.`);
        if (!a.zip_code?.trim())
            errs.push(`Previous address #${i + 1}: ZIP is required.`);
        if (!a.from_date)
            errs.push(`Previous address #${i + 1}: From date is required.`);
    }
    step2Errors.value = errs;
    if (errs.length) return;

    router.put(
        namedRoute('updateStep', { driver: props.driver!.id, step: 2 }),
        {
...
const step3 = reactive({
    ...
    applying_position_other:
        props.stepData?.step3?.applying_position_other ?? '',
    ...
    vehicle_assignment_type:
        props.stepData?.step3?.vehicle_assignment_type ??
        (null as string | null),
...
function submitStep3() {
    ...
    router.put(
        namedRoute('updateStep', { driver: props.driver!.id, step: 3 }),
        data,
    );
}
...
const step4 = reactive({
    ...
    experiences:
        ((props.stepData?.step4?.experiences ?? []) as any[]).length > 0
...
function submitStep4() {
    ...
    if (step4.licenses.length === 0) {
        ...
        step4.licenses.forEach((l, i) => {
            ...
            if (!l.license_number?.trim())
                errs.push(`License${n}: License number is required.`);
            if (!l.state_of_issue)
                errs.push(`License${n}: State of issue is required.`);
            if (!l.license_class)
                errs.push(`License${n}: License class is required.`);
            if (!l.expiration_date)
                errs.push(`License${n}: Expiration date is required.`);
        });
    }
    ...
    step4.licenses.forEach((l, i) => {
        ...
        if (Array.isArray(l.endorsements)) {
            l.endorsements.forEach((eid: number) =>
                data.append(`licenses[${i}][endorsements][]`, String(eid)),
            );
        }
    });
    licFrontFiles.value.forEach((file, i) => {
        if (file) data.append(`license_front_${i}`, file);
    });
    licBackFiles.value.forEach((file, i) => {
        if (file) data.append(`license_back_${i}`, file);
    });
    step4.experiences.forEach((e, i) => {
        if (!e.equipment_type) return;
        data.append(
            `experiences[${i}][equipment_type]`,
            String(e.equipment_type),
        );
        data.append(
            `experiences[${i}][years_experience]`,
            String(parseInt(e.years_experience) || 0),
        );
        data.append(
            `experiences[${i}][miles_driven]`,
            String(parseInt(e.miles_driven) || 0),
        );
        data.append(
            `experiences[${i}][requires_cdl]`,
            e.requires_cdl ? '1' : '0',
        );
    });
    data.append('_method', 'PUT');
    router.post(
        namedRoute('updateStep', { driver: props.driver!.id, step: 4 }),
        data,
    );
}
...
const step5 = reactive({
    ...
    medical_examiner_registry_number:
        props.stepData?.step5?.medical_examiner_registry_number ?? '',
    medical_card_expiration_date: toUsDate(
        props.stepData?.step5?.medical_card_expiration_date,
    ),
    medical_card: null as File | null,
    medical_card_url:
        props.stepData?.step5?.medical_card_url ?? (null as string | null),
});
...
function submitStep5() {
    ...
    data.append(
        'medical_examiner_registry_number',
        step5.medical_examiner_registry_number,
    );
    data.append(
        'medical_card_expiration_date',
        step5.medical_card_expiration_date,
    );
    if (step5.medical_card) data.append('medical_card', step5.medical_card);
    if (step5.social_security_card)
        data.append('social_security_card', step5.social_security_card);
    data.append('_method', 'PUT');
    router.post(
        namedRoute('updateStep', { driver: props.driver!.id, step: 5 }),
        data,
    );
}
...
const step6 = reactive({
    schools: ((props.stepData?.step6?.schools ?? []) as any[]).map(
        (s: any) => ({
            ...s,
            date_start: toUsDate(s.date_start),
            date_end: toUsDate(s.date_end),
            training_skills: Array.isArray(s.training_skills)
                ? [...s.training_skills]
                : ([] as string[]),
    ...
    courses: ((props.stepData?.step6?.courses ?? []) as any[]).map(
        (c: any) => ({
            ...c,
...
function onSchoolCertChange(e: Event, i: number) {
    ...
    if (file) {
        const reader = new FileReader();
        reader.onload = (ev) => {
            step6.schools[i].certificate_url = ev.target?.result as string;
        };
        reader.readAsDataURL(file);
    }
}
...
function onCourseCertChange(e: Event, i: number) {
    ...
    if (file) {
        const reader = new FileReader();
        reader.onload = (ev) => {
            step6.courses[i].certificate_url = ev.target?.result as string;
        };
        reader.readAsDataURL(file);
    }
}
...
function submitStep6() {
    ...
    step6.schools.forEach((s, i) => {
        ...
        const skills: string[] = Array.isArray((s as any).training_skills)
            ? (s as any).training_skills
            : [];
        skills.forEach((skill) =>
            data.append(`schools[${i}][training_skills][]`, skill),
        );
        if ((s as any).certificate_file) {
            data.append(
                `school_certificates[${i}]`,
                (s as any).certificate_file,
            );
        }
    });
    const courseFields = [
    ...
    step6.courses.forEach((c, i) => {
        ...
        if ((c as any).certificate_file) {
            data.append(
                `course_certificates[${i}]`,
                (c as any).certificate_file,
            );
        }
    });
    data.append('_method', 'PUT');
    router.post(
        namedRoute('updateStep', { driver: props.driver!.id, step: 6 }),
        data,
    );
}
...
const step7 = reactive({
    no_traffic_convictions:
        props.stepData?.step7?.no_traffic_convictions ?? false,
    convictions: ((props.stepData?.step7?.convictions ?? []) as any[]).map(
        (c: any) => ({
            ...c,
...
function onConvictionImageChange(e: Event, i: number) {
    ...
    if (file) {
        const reader = new FileReader();
        reader.onload = (ev) => {
            step7.convictions[i].image_url = ev.target?.result as string;
        };
        reader.readAsDataURL(file);
    }
}
...
function submitStep7() {
    ...
    data.append(
        'no_traffic_convictions',
        step7.no_traffic_convictions ? '1' : '0',
    );
    const convFields = [
        'id',
        'conviction_date',
    ...
    step7.convictions.forEach((c, i) => {
        ...
        if ((c as any).image_file)
            data.append(`conviction_images[${i}]`, (c as any).image_file);
    });
    data.append('_method', 'PUT');
    router.post(
        namedRoute('updateStep', { driver: props.driver!.id, step: 7 }),
        data,
    );
}
...
const step8 = reactive({
    ...
    accidents: ((props.stepData?.step8?.accidents ?? []) as any[]).map(
        (a: any) => ({
            ...a,
...
function onAccidentImageChange(e: Event, i: number) {
    ...
    if (file) {
        const reader = new FileReader();
        reader.onload = (ev) => {
            step8.accidents[i].image_url = ev.target?.result as string;
        };
        reader.readAsDataURL(file);
    }
}
...
function submitStep8() {
    ...
    step8.accidents.forEach((a, i) => {
        ...
        if ((a as any).image_file)
            data.append(`accident_images[${i}]`, (a as any).image_file);
    });
    data.append('_method', 'PUT');
    router.post(
        namedRoute('updateStep', { driver: props.driver!.id, step: 8 }),
        data,
    );
}
...
const step9 = reactive({
    ...
    has_positive_drug_test:
        props.stepData?.step9?.has_positive_drug_test ?? false,
    substance_abuse_professional:
        props.stepData?.step9?.substance_abuse_professional ?? '',
    ...
    recent_conviction_date: toUsDate(
        props.stepData?.step9?.recent_conviction_date,
    ),
    offense_details: props.stepData?.step9?.offense_details ?? '',
    consent_driving_record:
        props.stepData?.step9?.consent_driving_record ?? false,
});
...
function submitStep9() {
    const errs: string[] = [];
    if (step9.has_positive_drug_test && !step9.consent_to_release)
        errs.push(
            'You must consent to the release of drug/alcohol test information.',
        );
    if (!step9.consent_driving_record)
        errs.push('You must consent to the check of your driving record.');
    step9Errors.value = errs;
    if (errs.length) return;
    router.put(
        namedRoute('updateStep', { driver: props.driver!.id, step: 9 }),
        { ...step9 },
    );
}
...
const step10 = reactive({
    companies: ((props.stepData?.step10?.companies ?? []) as any[]).map(
        (c: any) => ({
            ...blankCompany(),
            ...c,
    ...
    unemployment_periods: (
        (props.stepData?.step10?.unemployment_periods ?? []) as any[]
    ).map((u: any) => ({
        ...blankUnemployment(),
        ...u,
    ...
    related_employments: (
        (props.stepData?.step10?.related_employments ?? []) as any[]
    ).map((r: any) => ({
        ...blankRelated(),
        ...r,
    ...
    has_unemployment_periods:
        ((props.stepData?.step10?.unemployment_periods ?? []) as any[]).length >
        0,
    has_correct_information:
...
function saveCompanyModal() {
    ...
    if (editingCompanyIdx.value !== null) {
        Object.assign(step10.companies[editingCompanyIdx.value], {
            ...companyForm,
        });
    } else {
...
function saveUnemploymentModal() {
    ...
    if (editingUnemploymentIdx.value !== null) {
        Object.assign(
            step10.unemployment_periods[editingUnemploymentIdx.value],
            { ...unemploymentForm },
        );
    } else {
...
function saveRelatedModal() {
    ...
    if (editingRelatedIdx.value !== null) {
        Object.assign(step10.related_employments[editingRelatedIdx.value], {
            ...relatedForm,
        });
    } else {
...
function removeRelated(i: number) {
    step10.related_employments.splice(i, 1);
}

function submitStep10() {
    if (!coverageStats.value.is_complete) {
        alert(
            'You must cover at least 10 years of employment history before continuing.',
        );
        return;
    }
    if (!step10.has_correct_information) {
        alert(
            'You must confirm that the information above is correct and contains no missing information.',
        );
        return;
    }
    router.put(
        namedRoute('updateStep', { driver: props.driver!.id, step: 10 }),
        {
            companies: step10.companies,
            unemployment_periods: step10.has_unemployment_periods
                ? step10.unemployment_periods
                : [],
...
function onSearchInput() {
    ...
    searchError.value = '';
    if (!companySearchTerm.value.trim()) {
        searchResults.value = [];
        return;
    }
...
const coverageStats = computed(() => {
    ...
    const empYears = step10.companies.reduce(
        (s: number, c: any) => s + calcYears(c.employed_from, c.employed_to),
        0,
    );
    const unempYears = step10.unemployment_periods.reduce(
        (s: number, u: any) => s + calcYears(u.start_date, u.end_date),
        0,
    );
    const relYears = step10.related_employments.reduce(
...
const combinedHistory = computed(() => {
    ...
    return items.sort((a, b) => {
        const da = a.from
            ? new Date(
                  a.from.replace(/(\d{2})\/(\d{2})\/(\d{4})/, '$3-$1-$2'),
              ).getTime()
            : 0;
        const db = b.from
            ? new Date(
                  b.from.replace(/(\d{2})\/(\d{2})\/(\d{4})/, '$3-$1-$2'),
              ).getTime()
            : 0;
        return db - da;
    });
...
async function sendEmail(company: any) {
    if (!company.id) {
        alert('Save the record first before sending email.');
        return;
    }
...
async function resendEmail(company: any) {
    ...
    try {
        await axios.post(
            namedRoute('employmentResendEmail', {
                driver: props.driver!.id,
                company: company.id,
            }),
        );
...
async function toggleEmailSent(company: any, sent: boolean) {
    ...
    try {
        await axios.post(
            namedRoute('employmentMarkEmailStatus', {
                driver: props.driver!.id,
                company: company.id,
            }),
            { sent },
...
const unsentEmailCount = computed(
    () =>
        step10.companies.filter((c: any) => c.email && !c.email_sent && c.id)
            .length,
);
...
const step11 = reactive({
    consent_all_policies_attached:
        props.stepData?.step11?.consent_all_policies_attached ?? false,
    substance_testing_consent:
        props.stepData?.step11?.substance_testing_consent ?? false,
    authorization_consent:
        props.stepData?.step11?.authorization_consent ?? false,
    fmcsa_clearinghouse_consent:
        props.stepData?.step11?.fmcsa_clearinghouse_consent ?? false,
...
function submitStep11() {
    const errs: string[] = [];
    if (!step11.consent_all_policies_attached)
        errs.push('You must agree to all policies attached.');
    if (!step11.substance_testing_consent)
        errs.push('You must consent to substance testing.');
    if (!step11.authorization_consent)
        errs.push('You must consent to authorization.');
    if (!step11.fmcsa_clearinghouse_consent)
        errs.push('You must consent to FMCSA Clearinghouse queries.');
    step11Errors.value = errs;
    if (errs.length) return;
    router.put(
        namedRoute('updateStep', { driver: props.driver!.id, step: 11 }),
        {
...
const step12 = reactive({
    has_criminal_charges:
        props.stepData?.step12?.has_criminal_charges ??
        (null as boolean | null),
    has_felony_conviction:
        props.stepData?.step12?.has_felony_conviction ??
        (null as boolean | null),
    has_minister_permit:
        props.stepData?.step12?.has_minister_permit ?? (null as boolean | null),
    fcra_consent: props.stepData?.step12?.fcra_consent ?? false,
    background_info_consent:
        props.stepData?.step12?.background_info_consent ?? false,
    // read-only display fields
    full_name:
        (props.stepData?.step12 as any)?.full_name ??
        props.driver?.name ??
        null,
    middle_name:
        (props.stepData?.step12 as any)?.middle_name ??
        props.driver?.middle_name ??
        null,
    last_name:
        (props.stepData?.step12 as any)?.last_name ??
        props.driver?.last_name ??
        null,
    date_of_birth:
        (props.stepData?.step12 as any)?.date_of_birth ??
        props.driver?.date_of_birth ??
        null,
...
function submitStep12() {
    const errs: string[] = [];
    if (step12.has_criminal_charges === null)
        errs.push('Please answer the criminal charges question.');
    if (step12.has_felony_conviction === null)
        errs.push('Please answer the felony conviction question.');
    if (!step12.fcra_consent) errs.push('You must accept the FCRA consent.');
    if (!step12.background_info_consent)
        errs.push('You must certify that the information is correct.');
    step12Errors.value = errs;
    if (errs.length) return;
    router.put(
        namedRoute('updateStep', { driver: props.driver!.id, step: 12 }),
        {
...
function submitStep13() {
    ...
    if (!step13.tax_classification)
        errs.push('Please select a tax classification.');
    if (step13.tax_classification === 'llc' && !step13.llc_classification)
        errs.push('LLC tax classification letter is required.');
    if (
        step13.tax_classification === 'other' &&
        !step13.other_classification.trim()
    )
        errs.push('Please specify the other classification.');
    ...
    if (errs.length) return;
    router.put(
        namedRoute('updateStep', { driver: props.driver!.id, step: 13 }),
        { ...step13 },
    );
}
...
function onTinInput(e: Event) {
    ...
    if (step13.tin_type === 'ssn') {
        if (digits.length > 5)
            step13.tin = `${digits.slice(0, 3)}-${digits.slice(3, 5)}-${digits.slice(5)}`;
        else if (digits.length > 3)
            step13.tin = `${digits.slice(0, 3)}-${digits.slice(3)}`;
...
function onZipInput(e: Event) {
    step13.zip_code = (e.target as HTMLInputElement).value
        .replace(/\D/g, '')
        .slice(0, 5);
}
function onExemptCodeInput(e: Event) {
    step13.exempt_payee_code = (e.target as HTMLInputElement).value
        .replace(/\D/g, '')
        .slice(0, 2);
}
...
const step14 = reactive({
    ...
    employment_history:
        (props.stepData?.step14 as any)?.employment_history ?? ([] as any[]),
});
...
function submitStep14() {
    ...
    if (!step14.is_accepted)
        errs.push(
            'You must certify that all information is true and complete.',
        );
    step14Errors.value = errs;
    if (errs.length) return;
    router.put(
        namedRoute('updateStep', { driver: props.driver!.id, step: 14 }),
        {
...
<template>
    <div>
        <Head
            :title="
                isEditMode ? 'Edit Driver – Wizard' : 'Register Driver – Wizard'
            "
        />
        <div class="p-0 sm:p-3">
            ...
            >
                <div>
                    <h1
                        class="text-xl font-semibold text-slate-800 sm:text-2xl dark:text-slate-100"
                    >
                        {{
                            isEditMode
                    ...
                    <p v-if="isEditMode" class="mt-1 text-sm text-slate-500">
                        {{ driver!.name }} {{ driver!.last_name }} &bull;
                        {{ driver!.carrier_name }}
                    </p>
                </div>
                <a
                    :href="namedRoute('index')"
                    class="flex shrink-0 items-center gap-1 text-sm text-slate-500 hover:text-slate-700"
                >
            ...
            <div v-if="isEditMode" class="mb-6">
                <div class="mb-1 flex justify-between text-xs text-slate-500">
                    ...
                    <span
                        >{{ Math.round((completedStep / totalSteps) * 100) }}%
                        completed</span
                    >
                </div>
                <div
            ...
            <div class="mb-6 overflow-x-auto">
                <div
                    class="flex min-w-max gap-1 border-b border-slate-200 pb-0 dark:border-slate-700"
                >
                    <button
                        v-for="step in steps"
                        :key="step.number"
                        ...
                        class="flex items-center gap-1.5 rounded-t-lg px-3 py-2.5 text-xs font-medium transition-colors"
                        :class="{
                            '-mb-px border border-slate-200 border-b-white bg-white text-primary dark:border-slate-600 dark:border-b-darkmode-600 dark:bg-darkmode-600':
                                currentStep === step.number,
                            'text-slate-500 hover:bg-slate-50 hover:text-slate-700':
                                currentStep !== step.number &&
                                canGoToStep(step.number),
                            'cursor-not-allowed text-slate-300': !canGoToStep(
                                step.number,
                            ),
                        }"
                    >
            ...
            <!-- Step Panels -->
            <div
                class="rounded-xl border border-slate-200 bg-white p-4 sm:p-6 dark:border-slate-600 dark:bg-darkmode-600"
            >
                <!-- ====================================================
                 STEP 1 – General Info
                 ==================================================== -->
                <div v-if="currentStep === 1">
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div class="md:col-span-2">
                            ...
                            >
                            <TomSelect
                                v-model="step1.carrier_id"
                                :disabled="
                                    props.carrierLocked ||
                                    (!!props.selectedCarrierId && !isEditMode)
                                "
                            >
                                <option
                                    v-for="c in props.carriers"
                                    :key="c.id"
                                    :value="c.id"
                                >
                                    {{ c.name }}
                            ...
                            <p
                                v-if="props.selectedCarrierId && !isEditMode"
                        ...
                        <div>
                            ...
                            />
                            <img
                                v-if="step1.photoPreview"
                                :src="step1.photoPreview"
                                class="mt-2 h-20 w-20 rounded-full border object-cover"
                            />
                        </div>
                    </div>
                    ...
                    <div class="mt-5 space-y-4">
                        <div
                            ...
                            :class="
                                errors.terms_accepted ? 'border-danger' : ''
                            "
                        >
                            <FormCheck.Input
                                v-model="step1.terms_accepted"
                                type="checkbox"
                                class="mt-0.5"
                            />
                            <div>
                                ...
                                <p class="mt-0.5 text-xs text-slate-500">
                                    Driver acknowledges and accepts all company
                                    policies
                                </p>
                            </div>
                        </div>
                        ...
                        >
                            <div class="flex items-start gap-3">
                                <FormCheck.Input
                                    v-model="step1.use_custom_dates"
                                    type="checkbox"
                                    class="mt-0.5"
                                />
                                <div>
                                    <p class="text-sm font-medium">
                                        ...
                                        <span class="font-normal text-slate-400"
                                            >(Only for Historical Drivers)</span
                                        >
                                    </p>
                                    <p class="mt-0.5 text-xs text-slate-500">
                                        Enable this option if you are
                                        registering a historical driver with
                                        specific registration dates.
                                    </p>
                                </div>
                            </div>
                            <div
                                v-if="step1.use_custom_dates"
                                class="mt-3 grid grid-cols-1 gap-3"
                            >
                                <div>
                                    <label
                                        class="mb-1 block text-xs font-medium"
                                        >Custom Registration Date</label
                                    >
                                    <Litepicker
                                        v-model="step1.custom_created_at"
                                        :options="lpOptions"
                                    />
                                </div>
                            </div>
                    ...
                    <div class="mt-6 flex justify-end">
                        <Button variant="primary" @click="submitStep1">
                            ...
                            <Lucide icon="ChevronRight" class="ml-1 h-4 w-4" />
                        </Button>
                    </div>
                </div>
                ...
                <div v-else-if="currentStep === 2">
                    ...
                    <div class="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <TomSelect v-model="step2.state">
                                ...
                                <option
                                    v-for="(name, code) in props.usStates"
                                    :key="code"
                                    :value="code"
                                >
                                    {{ name }}
                        ...
                        <div class="md:col-span-2">
                            <FormCheck>
                                <FormCheck.Input
                                    v-model="step2.lived_three_years"
                                    type="checkbox"
                                />
                                <FormCheck.Label
                                    >I have lived at this address for 3+
                                    years</FormCheck.Label
                                >
                            </FormCheck>
                        </div>
                    </div>
                    ...
                    <div v-if="!step2.lived_three_years">
                        ...
                        <div
                            v-for="(addr, i) in step2.previous_addresses"
                            :key="i"
                        ...
                        >
                            ...
                            <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                                ...
                                <FormInput
                                    v-model="addr.zip_code"
                                    placeholder="ZIP"
                                />
                                <Litepicker
                                    v-model="addr.from_date"
                                    :options="lpOptions"
                                />
                                <Litepicker
                                    v-model="addr.to_date"
                                    :options="lpOptions"
                                />
                            </div>
                        </div>
                    ...
                    <div class="mt-6 flex justify-between">
                        <Button
                            ...
                            @click="currentStep = 1"
                        >
                            <Lucide icon="ChevronLeft" class="mr-1 h-4 w-4" />
                            Previous
                        </Button>
                ...
                <div v-else-if="currentStep === 3">
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        ...
                        <div v-if="step3.applying_position === 'other'">
                            ...
                            >
                            <FormInput
                                v-model="step3.applying_position_other"
                            />
                        </div>
                        <div>
                            <label class="mb-1 block text-sm font-medium"
                                >Applying Location</label
                            >
                            <FormInput
                                v-model="step3.applying_location"
                                placeholder="City, State"
                            />
                        </div>
                        <div>
                            <label class="mb-1 block text-sm font-medium"
                                >Expected Pay ($/hr)</label
                            >
                            <FormInput
                                v-model="step3.expected_pay"
                                type="number"
                                step="0.01"
                            />
                        </div>
                    ...
                    <div class="mt-6">
                        ...
                        <p class="mb-3 text-sm text-slate-500">
                            Select the vehicle arrangement for this driver:
                        </p>
                                ...
                                v-for="opt in [
                                    {
                                        ...
                                        desc: 'Driver owns their vehicle',
                                    },
                                    {
                                        value: 'third_party',
                                        label: 'Third Party',
                                        desc: 'Vehicle from a third-party company',
                                    },
                                    {
                                        value: 'company',
                                        label: 'Company Driver',
                                        desc: 'Uses a company-owned vehicle',
                                    },
                                ]"
                                :key="opt.value"
                                class="flex cursor-pointer items-start gap-3 rounded-lg border p-4 transition"
                                :class="
                                    step3.vehicle_assignment_type === opt.value
                                        ? 'border-primary bg-primary/5'
                                        : errors.vehicle_assignment_type &&
                                            !step3.vehicle_assignment_type
                        ...
                        <p
                            v-if="errors.vehicle_assignment_type"
                            class="mt-1 text-xs text-danger"
                        >
                            {{ errors.vehicle_assignment_type }}
                        </p>
                        <p
                            v-else-if="!step3.vehicle_assignment_type"
                            class="mt-1 text-xs text-slate-400"
                        >
                    ...
                    <div
                        v-if="
                            step3.vehicle_assignment_type === 'owner_operator'
                        "
                        class="mt-4 rounded-lg border border-slate-200 p-4"
                    >
                        ...
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                            <div>
                                ...
                                >
                                <FormInput
                                    v-model="step3.owner_name"
                                    placeholder="Full name"
                                    :class="
                                        errors.owner_name ? 'border-danger' : ''
                                    "
                                />
                                <p
                    ...
                    <!-- Third Party Info -->
                    <div
                        v-if="step3.vehicle_assignment_type === 'third_party'"
                        class="mt-4 rounded-lg border border-slate-200 p-4"
                    >
                        <h4 class="mb-3 text-sm font-semibold text-primary">
                        ...
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                            <div>
                                ...
                                >
                                <FormInput
                                    v-model="step3.third_party_name"
                            ...
                            <div>
                                ...
                                >
                                <FormInput
                                    v-model="step3.third_party_phone"
                                    placeholder="Phone"
                                />
                            </div>
                            <div>
                                ...
                                >
                                <FormInput
                                    v-model="step3.third_party_email"
                                    type="email"
                                    placeholder="Email"
                                    :class="
                            ...
                            <div>
                                ...
                                >
                                <FormInput
                                    v-model="step3.third_party_address"
                                    placeholder="Address"
                                />
                            </div>
                            <div>
                                ...
                                >
                                <FormInput
                                    v-model="step3.third_party_contact"
                            ...
                            <div>
                                ...
                                >
                                <FormInput
                                    v-model="step3.third_party_fein"
                    ...
                    <div
                        v-if="step3.vehicle_assignment_type !== 'company'"
                        class="mt-6"
                    >
                        <h3 class="mb-3 text-base font-semibold">
                            Vehicle Information
                            <span class="text-xs font-normal text-slate-400"
                                >(vehicles registered by this driver)</span
                            >
                        </h3>

                        <!-- Pending new vehicle preview -->
                        <div
                            v-if="pendingNewVehicle"
                            class="mb-3 flex items-center gap-2 rounded-lg border border-success/30 bg-success/10 p-3 text-sm"
                        >
                            <Lucide
                                icon="CheckCircle"
                            ...
                            />
                            <span
                                >New vehicle to register:
                                <strong
                                    >{{ pendingNewVehicle.year }}
                                    {{ pendingNewVehicle.make }}
                                    {{ pendingNewVehicle.model }}</strong
                                >
                                — VIN: {{ pendingNewVehicle.vin || 'N/A'
                                }}{{
                                    pendingNewVehicle.company_unit_number
                                        ? ` — Unit: ${pendingNewVehicle.company_unit_number}`
                                        : ''
                                }}</span
                            >
                            <button
                                type="button"
                                class="ml-auto text-slate-400 hover:text-danger"
                                @click="pendingNewVehicle = null"
                            >
                                <Lucide icon="X" class="h-4 w-4" />
                            </button>
                        </div>
                        ...
                        <div
                            v-if="props.vehicles.length"
                            class="overflow-hidden rounded-lg border border-slate-200"
                        >
                            <div
                                class="bg-slate-50 px-4 py-2 text-xs font-semibold tracking-wide text-slate-500 uppercase"
                            >
                                Existing Vehicles
                            </div>
                            <table class="w-full text-sm">
                                ...
                                <tbody>
                                    <tr
                                        ...
                                        :key="v.id"
                                        class="border-b border-slate-100 transition last:border-0"
                                        :class="
                                            step3.vehicle_id === v.id
                                                ? 'bg-primary/5'
                                                : 'hover:bg-slate-50'
                                        "
                                    >
                        ...
                        <div v-else class="mb-3 text-sm text-slate-400 italic">
                            No vehicles registered for this carrier yet.
                        </div>
                        ...
                        >
                            <Lucide icon="Plus" class="mr-1 h-4 w-4" /> Register
                            New Vehicle
                        </Button>
                    </div>
                    ...
                    <div class="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div class="space-y-3 md:col-span-2">
                            ...
                            >
                                <FormCheck.Input
                                    v-model="step3.eligible_to_work"
                                    type="checkbox"
                                    class="mt-0.5"
                                />
                                <div>
                                    ...
                                    >
                                    <p
                                        v-if="errors.eligible_to_work"
                                        class="mt-0.5 text-xs text-danger"
                                    >
                                        {{ errors.eligible_to_work }}
                                    </p>
                                    <p v-else class="text-xs text-slate-400">
                                        Required — applicant must be legally
                            ...
                            <div class="flex flex-wrap gap-6">
                                <FormCheck>
                                    <FormCheck.Input
                                        v-model="step3.can_speak_english"
                                        type="checkbox"
                                    />
                                    <FormCheck.Label
                                        >Can speak English</FormCheck.Label
                                    >
                                </FormCheck>
                                <FormCheck>
                                    <FormCheck.Input
                                        v-model="step3.has_twic_card"
                                        type="checkbox"
                                    />
                                    <FormCheck.Label
                                        >Has TWIC Card</FormCheck.Label
                                    >
                                </FormCheck>
                            </div>
                        </div>
                        <div v-if="step3.has_twic_card">
                            <label class="mb-1 block text-sm font-medium"
                                >TWIC Expiration Date</label
                            >
                            <Litepicker
                                v-model="step3.twic_expiration_date"
                                :options="lpOptions"
                            />
                        </div>
                    </div>
                    ...
                    >
                        <Dialog.Panel>
                            <div class="max-h-[85vh] overflow-y-auto p-5">
                                <div class="mb-5 flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-slate-500">
                                            Fill in the vehicle details — type
                                            shown based on assignment type
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div
                                    class="mb-5 grid grid-cols-1 gap-4 md:grid-cols-3"
                                >
                                    <div>
                                        <label
                                    ...
                                    <div>
                                        ...
                                        <FormInput
                                            v-model="
                                                newVehicle.company_unit_number
                                            "
                                    ...
                                    <div>
                                        ...
                                        >
                                        <FormInput
                                            v-model="newVehicle.gvwr"
                                            placeholder="e.g. 80000 lbs"
                                        />
                                    </div>
                                    <div>
                                        <label
                                ...
                                >
                                    ...
                                    <div>
                                        ...
                                        <Litepicker
                                            v-model="
                                                newVehicle.registration_expiration_date
                                            "
                                            :options="lpOptions"
                                            class="w-full"
                                        />
                                    ...
                                    <div
                                        class="flex items-center gap-6 md:col-span-3"
                                    >
                                        <label
                                        ...
                                        >
                                            <FormCheck.Input
                                                type="checkbox"
                                                v-model="
                                                    newVehicle.irp_apportioned_plate
                                                "
                                            />
                                            <span class="text-sm"
                                                >IRP Apportioned Plate</span
                                            >
                                        </label>
                                    </div>
                                </div>
                                ...
                                >
                                    ...
                                    <div>
                                        ...
                                        <textarea
                                            ...
                                            rows="3"
                                            class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:ring-1 focus:ring-primary focus:outline-none"
                                ...
                                <div
                                    v-if="
                                        ...
                                        'owner_operator'
                                    "
                                    class="mb-5 rounded-lg border border-slate-200 bg-slate-50 p-3"
                                >
                                    <label
                                    ...
                                    >
                                        <FormCheck.Input
                                            type="checkbox"
                                            v-model="newVehicle.terms_accepted"
                                            class="mt-0.5"
                                        />
                                        <span class="text-sm text-slate-600">
                                            I confirm that the vehicle
                                            information provided is accurate and
                                            the owner has agreed to operate
                                ...
                                <div
                                    v-if="
                                        ...
                                        'third_party'
                                    "
                                    class="mb-5 flex items-start gap-2 rounded-lg border border-warning/30 bg-warning/10 p-3"
                                >
                                    <Lucide
                                        icon="Mail"
                                    ...
                                    <p class="text-warning-dark text-sm">
                                        A document signing request will be sent
                                        automatically to
                                        <strong>{{
                                            step3.third_party_email ||
                                            'the third party email'
                                        }}</strong>
                                        when this step is saved.
                                    </p>
                                </div>

                                <div
                                    class="flex justify-end gap-2 border-t border-slate-100 pt-2"
                                >
                                    <Button
                                        variant="outline-secondary"
                ...
                <div v-else-if="currentStep === 4">
                    <div class="mb-5 flex items-center justify-between">
                        ...
                        <Button
                            ...
                            @click="addLicense"
                        >
                            <Lucide icon="Plus" class="mr-1 h-3.5 w-3.5" /> Add
                            License
                        </Button>
                    </div>

                    <div
                        v-for="(lic, i) in step4.licenses"
                        :key="i"
                        class="mb-4 rounded-lg border border-slate-200 p-4"
                    >
                        ...
                        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                            ...
                            <div class="flex items-center pt-5">
                                <FormCheck>
                                    ...
                                    />
                                    <FormCheck.Label
                                        >This is a Commercial Driver's License
                                        (CDL)</FormCheck.Label
                                    >
                                </FormCheck>
                            </div>
                        ...
                        >
                            ...
                            <div class="grid grid-cols-2 gap-2 md:grid-cols-3">
                                <FormCheck
                                    v-for="end in props.endorsements"
                                    :key="end.id"
                                >
                                    <FormCheck.Input
                                        type="checkbox"
                                        :value="end.id"
                                        :checked="
                                            lic.endorsements.includes(end.id)
                                        "
                                        @change="
                                            (e: Event) => {
                                                const checked = (
                                                ...
                                                if (checked) {
                                                    if (
                                                        !lic.endorsements.includes(
                                                            end.id,
                                                        )
                                                    )
                                                        lic.endorsements.push(
                                                            end.id,
                                                        );
                                                } else {
                                                    lic.endorsements =
                                                        lic.endorsements.filter(
                                                            (id: number) =>
                                                                id !== end.id,
                                                        );
                                                }
                            ...
                            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                                <div>
                                    ...
                                    <input
                                        ...
                                        @change="(e) => onLicFront(e, i)"
                                        :class="fileInputClass"
                                    />
                                    <img
                                        v-if="licFrontPreviews[i]"
                                        :src="licFrontPreviews[i]!"
                                ...
                                <div>
                                    ...
                                    <input
                                        type="file"
                                        accept="image/*,application/pdf"
                                        @change="(e) => onLicBack(e, i)"
                                        :class="fileInputClass"
                                    />
                                    <img
                                        v-if="licBackPreviews[i]"
                                        :src="licBackPreviews[i]!"
                    ...
                    <div class="mt-6">
                        ...
                        <div
                            v-for="(exp, i) in step4.experiences"
                            :key="i"
                            class="mb-3 rounded-lg border border-slate-200 p-4"
                        >
                            <div class="mb-3 flex justify-between">
                                ...
                                >
                                <button
                                    v-if="step4.experiences.length > 1"
                    ...
                    <!-- Validation errors -->
                    <div
                        v-if="step4Errors.length"
                        class="mb-4 rounded-xl border border-red-200 bg-red-50 p-4"
                    >
                        <ul class="list-inside list-disc space-y-1">
                            <li
                                v-for="e in step4Errors"
                                :key="e"
                                class="text-sm text-red-700"
                            >
                                {{ e }}
                ...
                <div v-else-if="currentStep === 5">
                    ...
                    <div class="mb-5 rounded-lg border border-slate-200 p-4">
                        ...
                        >
                            <Lucide icon="ShieldCheck" class="h-4 w-4" /> Social
                            Security
                        </h3>
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                            <div>
                                ...
                                >
                                <FormInput
                                    v-model="step5.social_security_number"
                            ...
                            <div>
                                ...
                                />
                                <a
                                    v-if="step5.ss_card_url"
                                    :href="step5.ss_card_url"
                                    target="_blank"
                                    class="mt-1 block text-xs text-primary"
                                >
                                    <Lucide
                    ...
                    <div class="mb-5 rounded-lg border border-slate-200 p-4">
                        ...
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                            <div>
                                ...
                                >
                                <FormInput
                                    v-model="step5.medical_examiner_name"
                            ...
                            <div>
                                ...
                                >
                                <Litepicker
                                    v-model="step5.medical_card_expiration_date"
                                    :options="lpOptions"
                                />
                            </div>
                            <div>
                                <label class="mb-1 block text-xs font-medium"
                                    >Medical Card Document</label
                                >
                                <input
                                    type="file"
                                    accept="image/*,application/pdf"
                                ...
                                />
                                <a
                                    v-if="step5.medical_card_url"
                                    :href="step5.medical_card_url"
                                    target="_blank"
                                    class="mt-1 block text-xs text-primary"
                                >
                                    <Lucide
                ...
                <div v-else-if="currentStep === 6">
                    ...
                    <div
                        v-if="step6.schools.length === 0"
                        class="rounded-lg border border-dashed py-8 text-center text-sm text-slate-400"
                    >
                        No schools added yet. Click "Add School" to add one.
                    </div>

                    <div
                        v-for="(school, si) in step6.schools"
                        :key="si"
                        class="mb-4 rounded-lg border border-slate-200 p-4"
                    >
                        <div class="mb-3 flex justify-between">
                        ...
                        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                            ...
                            <div
                                class="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:gap-x-6 sm:gap-y-2 md:col-span-2"
                            >
                                <FormCheck>
                                    <FormCheck.Input
                                        v-model="school.graduated"
                                        type="checkbox"
                                    />
                                ...
                                <FormCheck>
                                    ...
                                    <FormCheck.Label
                                        >Performed Safety
                                        Functions</FormCheck.Label
                                    >
                                </FormCheck>
                            </div>
                            <div class="md:col-span-2">
                                <label class="mb-2 block text-xs font-medium"
                                    >Which skills were trained in this program?
                                    <span class="font-normal text-slate-400"
                                ...
                                >
                                    <FormCheck
                                        v-for="skill in trainingSkillOptions"
                                        :key="skill.value"
                                    >
                                        <FormCheck.Input
                                            type="checkbox"
                                            :value="skill.value"
                                            :checked="
                                                school.training_skills?.includes(
                                                    skill.value,
                                                )
                                            "
                                            @change="
                                                (e: Event) => {
                                                    ...
                                                    if (el.checked) {
                                                        if (
                                                            ...
                                                            school.training_skills.push(
                                                                skill.value,
                                                            );
                                                    } else {
                                                        school.training_skills =
                                                            school.training_skills.filter(
                                                                (v: string) =>
                                                                    v !==
                                                                    skill.value,
                                                            );
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input
                                    type="file"
                                    accept=".pdf,image/*"
                                    @change="(e) => onSchoolCertChange(e, si)"
                                    :class="fileInputClass"
                                />
                                <a
                                    v-if="
                                        school.certificate_url &&
                                        !school.certificate_file
                                    "
                                ...
                                >
                                    ...
                                    />
                                    New file selected:
                                    {{ (school.certificate_file as File).name }}
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-4">
                        ...
                        <div
                            v-for="(course, ci) in step6.courses"
                            :key="ci"
                        ...
                        >
                            ...
                            <div class="grid grid-cols-1 gap-2 md:grid-cols-3">
                                <div class="md:col-span-2">
                                    ...
                                    >
                                    <FormInput
                                        v-model="course.organization_name"
                                    />
                                </div>
                                <div>
                                    <label
                                ...
                                <div>
                                    ...
                                    >
                                    <Litepicker
                                        v-model="course.certification_date"
                                        :options="lpOptions"
                                    />
                                </div>
                                <div>
                                    ...
                                    >
                                    <Litepicker
                                        v-model="course.expiration_date"
                                        :options="lpOptions"
                                    />
                                </div>
                                <div>
                                    ...
                                    >
                                    <FormInput
                                        v-model="course.years_experience"
                                        type="number"
                                        step="0.5"
                                    />
                                </div>
                                <div class="md:col-span-3">
                                    ...
                                    >
                                        ...
                                        New file selected:
                                        {{
                                            (course.certificate_file as File)
                                                .name
                                        }}
                                    </span>
                                </div>
                ...
                <div v-else-if="currentStep === 7">
                    ...
                    <FormCheck class="mb-4">
                        <FormCheck.Input
                            v-model="step7.no_traffic_convictions"
                            type="checkbox"
                            @change="step7.convictions = []"
                        />
                        <FormCheck.Label
                            >No traffic convictions in the past 3
                            years</FormCheck.Label
                        >
                    </FormCheck>
                    ...
                    <div v-if="!step7.no_traffic_convictions">
                        ...
                        <div
                            v-for="(c, i) in step7.convictions"
                            :key="i"
                            class="mb-3 rounded-lg border border-slate-200 p-4"
                        >
                            <div class="mb-2 flex justify-between">
                            ...
                            <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <input
                                        ...
                                        accept=".pdf,image/*"
                                        @change="
                                            (e) => onConvictionImageChange(e, i)
                                        "
                                        :class="fileInputClass"
                                    />
                                    <a
                                        v-if="c.image_url && !c.image_file"
                                        :href="c.image_url"
                                        target="_blank"
                                        class="mt-1 inline-flex items-center gap-1 text-xs text-primary hover:underline"
                                    >
                ...
                <div v-else-if="currentStep === 8">
                    <h2 class="mb-5 text-lg font-semibold">Accident Record</h2>
                    <FormCheck class="mb-4">
                        <FormCheck.Input
                            v-model="step8.no_accidents"
                            type="checkbox"
                            @change="step8.accidents = []"
                        />
                        <FormCheck.Label
                            >No accidents in the past 3 years</FormCheck.Label
                        >
                    </FormCheck>
                        ...
                        >
                            ...
                            <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                                ...
                                >
                                    <FormCheck>
                                        <FormCheck.Input
                                            v-model="a.had_fatalities"
                                            type="checkbox"
                                        />
                                        <FormCheck.Label
                                            >Involved
                                            Fatalities</FormCheck.Label
                                        >
                                    </FormCheck>
                                    <FormCheck>
                                        <FormCheck.Input
                                            v-model="a.had_injuries"
                                            type="checkbox"
                                        />
                                        <FormCheck.Label
                                            >Involved Injuries</FormCheck.Label
                                        >
                                    </FormCheck>
                                </div>
                                <div v-if="a.had_fatalities">
                                    ...
                                    >
                                    <FormInput
                                        v-model.number="a.number_of_fatalities"
                                        type="number"
                                        min="0"
                                    />
                                </div>
                                <div v-if="a.had_injuries">
                                    ...
                                    >
                                    <FormInput
                                        v-model.number="a.number_of_injuries"
                                        type="number"
                                        min="0"
                                    />
                                </div>
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <input
                                        ...
                                        accept=".pdf,image/*"
                                        @change="
                                            (e) => onAccidentImageChange(e, i)
                                        "
                                        :class="fileInputClass"
                                    />
                                    <a
                                        v-if="a.image_url && !a.image_file"
                                        :href="a.image_url"
                                        target="_blank"
                                        class="mt-1 inline-flex items-center gap-1 text-xs text-primary hover:underline"
                                    >
                ...
                <div v-else-if="currentStep === 9">
                    ...
                    <p class="mb-6 text-sm text-slate-500">
                        Answer each question truthfully. A "Yes" answer does not
                        automatically disqualify an applicant. If "Yes", provide
                        additional details in the field that appears.
                    </p>
                    ...
                    <div class="space-y-4">
                        ...
                        >
                            ...
                            <div
                                v-if="step9.is_disqualified"
                                class="border-t border-amber-100 bg-amber-50 px-4 pb-4"
                            >
                                <label
                                    class="mt-3 mb-1 block text-xs font-medium"
                                    >Please provide details
                                    <span class="text-danger">*</span></label
                                >
                                <textarea
                                    v-model="step9.disqualified_details"
                                    rows="3"
                                    class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:ring-1 focus:ring-primary focus:outline-none"
                                    placeholder="State, date, reason, and resolution..."
                                />
                            </div>
                        </div>
                        ...
                        >
                            <div class="flex items-start gap-3 p-4">
                                <FormCheck.Input
                                    v-model="step9.is_license_suspended"
                                    type="checkbox"
                                    class="mt-0.5 shrink-0"
                                />
                                <div>
                                    <p class="text-sm font-medium">
                                        Has your license, permit, or privilege
                                        to drive ever been suspended or revoked
                                        for any reason?
                                        <span class="font-normal text-slate-400"
                                            >[49 CFR 391.21(b)(9)]</span
                                        >
                                    </p>
                                </div>
                            </div>
                            <div
                                v-if="step9.is_license_suspended"
                                class="border-t border-amber-100 bg-amber-50 px-4 pb-4"
                            >
                                <label
                                    class="mt-3 mb-1 block text-xs font-medium"
                                ...
                                >
                                <textarea
                                    v-model="step9.suspension_details"
                                    rows="3"
                                    class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:ring-1 focus:ring-primary focus:outline-none"
                                    placeholder="State, date, reason, and resolution..."
                                />
                        ...
                        >
                            <div class="flex items-start gap-3 p-4">
                                <FormCheck.Input
                                    v-model="step9.is_license_denied"
                                    type="checkbox"
                                    class="mt-0.5 shrink-0"
                                />
                                <div>
                            ...
                            >
                                ...
                                >
                                <textarea
                                    v-model="step9.denial_details"
                                    rows="3"
                                    class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:ring-1 focus:ring-primary focus:outline-none"
                                    placeholder="State, date, and reason..."
                                />
                            </div>
                        </div>
                        ...
                        >
                            <div class="flex items-start gap-3 p-4">
                                <FormCheck.Input
                                    v-model="step9.has_positive_drug_test"
                                    type="checkbox"
                                    class="mt-0.5 shrink-0"
                                />
                                <div>
                                    <p class="text-sm font-medium">
                                        ...
                                        tested positive, or refused to test, on
                                        a pre-employment drug or alcohol test by
                                        an employer to whom you applied, but did
                                        not obtain, safety-sensitive
                                        transportation work covered by DOT
                            ...
                            <div
                                v-if="step9.has_positive_drug_test"
                                class="border-t border-amber-100 bg-amber-50 px-4 pb-4"
                            >
                                <p class="mt-3 mb-2 text-xs text-slate-500">
                                    If yes, please provide the name of the
                                    Substance Abuse Professional (SAP) that
                                    evaluated you below, along with the name of
                                    the agency that performed your return to
                                    duty test.
                                </p>
                                <div
                                ...
                                >
                                    <div>
                                        <label
                                            class="mb-1 block text-xs font-medium"
                                            >Substance Abuse Professional</label
                                        >
                                        <FormInput
                                            v-model="
                                                step9.substance_abuse_professional
                                            "
                                ...
                                <div class="mt-3">
                                    <label
                                        class="mb-1 block text-xs font-medium"
                                        >Return to Duty Test Agency</label
                                    >
                                    <FormInput
                                        v-model="step9.return_duty_agency"
                                        placeholder="Enter agency name"
                                    />
                                </div>
                                <div
                                    class="mt-4 border-t border-amber-200 pt-3"
                                >
                                    <p
                                        class="mb-2 text-xs text-slate-500 italic"
                                    >
                                        *If you answered yes to the above
                                        question please agree to Consent for
                                        Release of Information regarding
                                        Previous Pre-Employment Controlled
                                        Substances or Alcohol Testing form.*
                                    </p>
                                    <div class="flex items-start gap-2">
                                        <FormCheck.Input
                                            v-model="step9.consent_to_release"
                                            type="checkbox"
                                            class="mt-0.5 shrink-0"
                                        />
                                        <p class="text-sm">
                                            Do you agree and consent to the
                                            above?
                                        </p>
                                    </div>
                                </div>
                        ...
                        >
                            <div class="flex items-start gap-3 p-4">
                                <FormCheck.Input
                                    v-model="step9.has_duty_offenses"
                                    type="checkbox"
                                    class="mt-0.5 shrink-0"
                                />
                                <div>
                                    <p class="text-sm font-medium">
                                        In the past three (3) years, have you
                                        ever been convicted of any of the
                                        following offenses committed during
                                        on-duty time?
                                        <span class="font-normal text-slate-400"
                                            >[49 CFR 391.15 and 49 CFR
                                            395.2]</span
                                        >
                                    </p>
                                </div>
                            ...
                            >
                                ...
                                >
                                    ...
                                    <div>
                                        ...
                                        >
                                        <textarea
                                            v-model="step9.offense_details"
                                            rows="3"
                                            class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:ring-1 focus:ring-primary focus:outline-none"
                                            placeholder="Describe the offense(s)..."
                                        />
                                    </div>
                                </div>
                            </div>
                        ...
                        >
                            ...
                            <p class="text-sm text-slate-600">
                                I understand that according to the Federal Motor
                                Carrier Safety Regulations, my previous driving
                                record will be investigated and that my
                                employment is subject to satisfactory reports
                                from previous employers and other sources.
                            </p>
                            <div class="flex items-start gap-3 pt-1">
                                <FormCheck.Input
                                    v-model="step9.consent_driving_record"
                                    type="checkbox"
                                    class="mt-0.5 shrink-0"
                                />
                                <p class="text-sm font-medium">
                    ...
                    <!-- Validation errors -->
                    <div
                        v-if="step9Errors.length"
                        class="mb-4 rounded-xl border border-red-200 bg-red-50 p-4"
                    >
                        <ul class="list-inside list-disc space-y-1">
                            <li
                                v-for="e in step9Errors"
                                :key="e"
                                class="text-sm text-red-700"
                            >
                                {{ e }}
                ...
                <div v-else-if="currentStep === 10">
                    ...
                    <!-- Regulatory notice -->
                    <div
                        class="mb-5 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-slate-700"
                    >
                        <p>
                            <strong
                                >All driver applicants must provide information
                                on all work references during the preceding
                                <span class="font-bold"
                                    >three (3) years</span
                                ></strong
                            >
                            from the date application is submitted. Those
                            applying to operate a
                            <strong>commercial motor vehicle</strong> as defined
                            in §383.5 (requiring a CDL) shall provide
                            <strong>ten (10) years</strong> of employment
                            history.
                        </p>
                        <p class="mt-2">
                            <strong
                                >NOTE: Please list companies in reverse order
                                starting with the most recent and leave no gaps
                                in employment history.</strong
                            >
                        </p>
                    </div>
                    ...
                    <div
                        v-if="unsentEmailCount > 0"
                        class="mb-4 flex flex-col gap-3 rounded-md border border-blue-200 bg-blue-50 p-3 sm:flex-row sm:items-center sm:justify-between"
                    >
                        <div
                        ...
                        >
                            ...
                            />
                            <span
                                ><strong>{{ unsentEmailCount }}</strong>
                                verification
                                {{
                                    unsentEmailCount === 1 ? 'email' : 'emails'
                                }}
                                ready to send</span
                            >
                        </div>
                        <Button
                            ...
                            @click="
                                step10.companies
                                    .filter(
                                        (c: any) =>
                                            c.email && !c.email_sent && c.id,
                                    )
                                    .forEach((c: any) => sendEmail(c))
                            "
                        >
                            <Lucide icon="Send" class="mr-1 h-3.5 w-3.5" /> Send
                            All Verification Emails
                        </Button>
                    </div>
                    ...
                    <div class="mb-5">
                        ...
                        <div
                            v-if="combinedHistory.length === 0"
                            class="rounded-lg border border-dashed py-5 text-center text-sm text-slate-400"
                        >
                            No employment records yet. Use the buttons below to
                            add companies, unemployment periods, or other
                            positions.
                        </div>
                        <!-- Desktop: table view -->
                        <div
                            v-else
                            class="hidden overflow-x-auto rounded-lg border border-slate-200 md:block"
                        >
                            <table class="min-w-full text-sm">
                                ...
                                <tbody class="divide-y divide-slate-100">
                                    <tr
                                        v-for="(item, i) in combinedHistory"
                                        :key="i"
                                        class="hover:bg-slate-50"
                                    >
                                        <td class="px-4 py-2">
                                            <span
                                                v-if="item.type === 'employed'"
                                                class="inline-flex items-center rounded bg-green-100 px-2 py-0.5 text-xs font-semibold text-green-800"
                                                >EMPLOYED</span
                                            >
                                            <span
                                                v-else-if="
                                                    item.type === 'related'
                                                "
                                                class="inline-flex items-center rounded bg-blue-100 px-2 py-0.5 text-xs font-semibold text-blue-800"
                                                >RELATED EMPLOYMENT</span
                                            >
                                            <span
                                                v-else
                                                class="inline-flex items-center rounded bg-yellow-100 px-2 py-0.5 text-xs font-semibold text-yellow-800"
                                                >UNEMPLOYED</span
                                            >
                                        </td>
                                        <td class="px-4 py-2 font-medium">
                                        ...
                                        <td class="px-4 py-2">
                                            ...
                                            >
                                                ...
                                                >
                                                <span
                                                    v-else-if="item.email_sent"
                                                    class="inline-flex items-center rounded bg-green-100 px-2 py-0.5 text-xs font-semibold text-green-700"
                                                    >Sent</span
                                                >
                                                <span
                                                    v-else
                                                    class="inline-flex items-center rounded bg-yellow-100 px-2 py-0.5 text-xs font-semibold text-yellow-700"
                                                    >Not Sent</span
                                                >
                                            </template>
                                            <span
                                                v-else
                                                class="inline-flex items-center rounded bg-slate-100 px-2 py-0.5 text-xs text-slate-400"
                                                >N/A</span
                                            >
                                        </td>
                                        <td class="px-4 py-2 text-right">
                                            ...
                                            >
                                                <button
                                                    ...
                                                    "
                                                    class="flex items-center gap-1 rounded border border-primary px-2 py-1 text-xs text-primary hover:opacity-70"
                                                >
                                                    <Lucide
                                                        icon="Pencil"
                                                        class="h-3 w-3"
                                                ...
                                                <button
                                                    ...
                                                    "
                                                    class="flex items-center gap-1 rounded border border-danger px-2 py-1 text-xs text-danger hover:opacity-70"
                                                >
                                                    <Lucide
                                                        icon="Trash2"
                                                        class="h-3 w-3"
                                                ...
                                                >
                                                    ...
                                                    <button
                                                        ...
                                                        "
                                                        class="flex items-center gap-1 rounded border border-orange-400 px-2 py-1 text-xs text-orange-500 hover:opacity-70"
                                                    ...
                                                    <button
                                                        ...
                                                        "
                                                        class="flex items-center gap-1 rounded border border-slate-300 px-2 py-1 text-xs text-slate-500 hover:opacity-70"
                                                        title="Mark as sent"
                                                    >
                                                        <Lucide
                                                            icon="Check"
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    <span
                                        v-if="item.type === 'employed'"
                                        class="inline-flex items-center rounded bg-green-100 px-2 py-0.5 text-[10px] font-semibold text-green-800"
                                        >EMPLOYED</span
                                    >
                                    <span
                                        v-else-if="item.type === 'related'"
                                        class="inline-flex items-center rounded bg-blue-100 px-2 py-0.5 text-[10px] font-semibold text-blue-800"
                                        >RELATED</span
                                    >
                                    <span
                                        v-else
                                        class="inline-flex items-center rounded bg-yellow-100 px-2 py-0.5 text-[10px] font-semibold text-yellow-800"
                                        >UNEMPLOYED</span
                                    >
                                    ...
                                    <template v-if="item.type === 'employed'">
                                        <span
                                            v-if="!item.email"
                                            class="inline-flex items-center rounded bg-slate-100 px-2 py-0.5 text-[10px] font-medium text-slate-500"
                                            >No Email</span
                                        >
                                        <span
                                            v-else-if="item.email_sent"
                                            class="inline-flex items-center rounded bg-green-100 px-2 py-0.5 text-[10px] font-semibold text-green-700"
                                            >Sent</span
                                        >
                                        <span
                                            v-else
                                            class="inline-flex items-center rounded bg-yellow-100 px-2 py-0.5 text-[10px] font-semibold text-yellow-700"
                                            >Not Sent</span
                                        >
                                    </template>
                                </div>
                                ...
                                <div
                                    class="mt-3 flex flex-wrap gap-1 border-t border-slate-100 pt-3"
                                >
                                    <button
                                        @click="
                                            item.type === 'employed'
                                                ? openEditCompany(item.idx)
                                                : item.type === 'unemployed'
                                                  ? openEditUnemployment(
                                                        item.idx,
                                                    )
                                                  : openEditRelated(item.idx)
                                        "
                                        class="flex items-center gap-1 rounded border border-primary px-2 py-1 text-xs text-primary hover:opacity-70"
                                    >
                                    ...
                                    <button
                                        @click="
                                            item.type === 'employed'
                                                ? removeCompany(item.idx)
                                                : item.type === 'unemployed'
                                                  ? removeUnemployment(item.idx)
                                                  : removeRelated(item.idx)
                                        "
                                        class="flex items-center gap-1 rounded border border-danger px-2 py-1 text-xs text-danger hover:opacity-70"
                                    >
                                    ...
                                    >
                                        ...
                                        >
                                            <Lucide
                                                ...
                                                class="h-3 w-3"
                                            />
                                            <span v-if="emailLoading[item.id]"
                                                >...</span
                                            >
                        ...
                        >
                            <div class="mb-2 flex items-center justify-between">
                                <span
                                    ...
                                    >Employment History Coverage</span
                                >
                                <span
                                    :class="
                                        coverageStats.is_complete
                                            ? 'text-success'
                                            : 'text-danger'
                                    "
                                    class="text-xs font-semibold"
                                >
                                    {{ coverageStats.total_years }} /
                                    {{ coverageStats.required_years }} years ({{
                                        coverageStats.coverage_percentage
                                    }}%)
                                </span>
                            </div>
                            <div
                                class="mb-2 h-2.5 w-full rounded-full bg-slate-200"
                            >
                                <div
                                    :class="
                                        coverageStats.is_complete
                            ...
                            <div
                                class="grid grid-cols-1 gap-1 text-xs text-slate-500 sm:grid-cols-3 sm:gap-2"
                            >
                                <span
                                    >Employment:
                                    <strong
                                        >{{
                                            coverageStats.employment_years
                                        }}
                                        yrs</strong
                                    ></span
                                >
                                <span
                                    ...
                                    <strong
                                        >{{
                                            coverageStats.unemployment_years
                                        }}
                                        yrs</strong
                                    ></span
                                >
                                <span
                                    ...
                                    <strong
                                        >{{
                                            coverageStats.related_employment_years
                                        }}
                                        yrs</strong
                                    ></span
                                >
                            ...
                            <div
                                v-if="!coverageStats.is_complete"
                                class="mt-2 rounded border border-amber-200 bg-amber-50 p-2 text-xs text-amber-700"
                            >
                                You need
                                {{
                                    coverageStats.required_years -
                                    coverageStats.total_years
                                }}
                                more years to meet the minimum requirement. Add
                                more employment, unemployment periods, or
                                related employment.
                            </div>
                        </div>
                    ...
                    <div class="mb-4 rounded-lg border border-slate-200 p-4">
                        ...
                        >
                            <div class="flex flex-1 items-center gap-3">
                                <FormCheck.Input
                                    v-model="step10.has_unemployment_periods"
                                    type="checkbox"
                                    id="has_unemployment"
                                />
                                <label
                                    for="has_unemployment"
                                    class="flex-1 cursor-pointer text-sm font-medium"
                                    >Have you been unemployed at any time within
                                    the last 10 years?</label
                                >
                            </div>
                            <Button
                                v-if="step10.has_unemployment_periods"
                                size="sm"
                                variant="outline-secondary"
                                class="w-full shrink-0 sm:w-auto"
                                @click="openAddUnemployment"
                            >
                                <Lucide icon="Plus" class="mr-1 h-3.5 w-3.5" />
                        ...
                        <div
                            v-if="
                                step10.has_unemployment_periods &&
                                step10.unemployment_periods.length === 0
                            "
                            class="mt-3 rounded-md border border-dashed py-3 text-center text-xs text-slate-400"
                        >
                            No unemployment periods added yet. Click "Add
                            Unemployment Period" to add one.
                        </div>
                        <div
                            v-else-if="step10.has_unemployment_periods"
                            class="mt-2 text-xs text-slate-500"
                        >
                            {{ step10.unemployment_periods.length }} period(s)
                            added — visible in the summary table above.
                        </div>
                    </div>
                    ...
                    <div class="rounded-lg border border-slate-200 p-4">
                        ...
                        >
                            <div>
                                ...
                                <p class="mt-0.5 text-xs text-slate-500">
                                    Other positions (Cook, Warehouseman,
                                    Carpenter, Clerk…) not part of your regular
                                    employment history. Count toward the 10-year
                                    requirement.
                                </p>
                            </div>
                            <Button
                                size="sm"
                                variant="outline-secondary"
                                class="w-full shrink-0 sm:ml-4 sm:w-auto"
                        ...
                        <div
                            v-if="step10.related_employments.length === 0"
                            class="mt-3 rounded-md border border-dashed py-3 text-center text-xs text-slate-400"
                        >
                            No other positions added yet.
                        </div>
                        <div v-else class="mt-2 text-xs text-slate-500">
                            {{ step10.related_employments.length }} position(s)
                            added — visible in the summary table above.
                        </div>
                    </div>
                    ...
                    <div class="mt-5 rounded-lg border border-slate-200 p-4">
                        <div class="flex items-start gap-3">
                            ...
                            >
                                ...
                                Is the information above correct and contains no
                                missing information?
                            </label>
                        </div>
                        <p
                            v-if="!coverageStats.is_complete"
                            class="mt-2 flex items-center gap-1 text-xs text-amber-600"
                        >
                            <Lucide
                                icon="AlertTriangle"
                                class="h-3.5 w-3.5 shrink-0"
                            />
                            This confirmation requires at least 10 years of
                            employment history coverage (currently
                            {{ coverageStats.total_years }} yrs).
                        </p>
                    </div>
                    ...
                    <div class="mt-4 flex justify-between">
                        ...
                        <Button
                            ...
                            :disabled="
                                !coverageStats.is_complete ||
                                !step10.has_correct_information
                            "
                        >
                            Save & Continue
                            <Lucide icon="ChevronRight" class="ml-1 h-4 w-4" />
                            ...
                            <div class="p-6">
                                ...
                                >
                                    <div>
                                        <h3 class="text-lg font-semibold">
                                            Search Previous Employer
                                        </h3>
                                        <p
                                            class="mt-0.5 text-xs text-slate-500"
                                        >
                                            Find and select a company from the
                                            database
                                        </p>
                                    </div>
                                    <button
                                ...
                                <div class="mb-4">
                                    ...
                                    <div class="relative">
                                        ...
                                        <input
                                            ...
                                            @input="onSearchInput"
                                            type="text"
                                            class="w-full rounded-md border border-slate-300 py-2.5 pr-4 pl-9 text-sm focus:ring-1 focus:ring-primary focus:outline-none"
                                        ...
                                        >
                                            ...
                                            >
                                                <div class="min-w-0">
                                                    ...
                                                    <p
                                                        class="mt-0.5 text-xs break-words text-slate-500"
                                                    >
                                                        {{
                                                            [
                                                                c.address,
                                                ...
                                                <div
                                                    class="shrink-0 text-xs break-words text-slate-400 sm:text-right"
                                                >
                                                    <p v-if="c.phone">
                                                        📞 {{ c.phone }}
                                                    </p>
                                ...
                                >
                                    <p class="text-xs text-slate-400">
                                        Can't find the company? Use "Add New
                                        Employment" to enter it manually.
                                    </p>
                                    <Button
                                        variant="outline-secondary"
                                        @click="showSearchModal = false"
                    ...
                    >
                        <Dialog.Panel class="max-h-[90vh] overflow-y-auto">
                            <div class="p-6">
                                ...
                                >
                                    <div>
                                        ...
                                        <p
                                            class="mt-0.5 text-xs text-slate-500"
                                        >
                                            Fill in the employment details below
                                        </p>
                                    </div>
                                    <button
                                ...
                                >
                                    <p
                                        class="mb-3 flex items-center gap-1 text-xs font-semibold tracking-wide text-slate-600 uppercase"
                                    >
                                        <Lucide
                                            icon="Building2"
                                            class="h-3.5 w-3.5"
                                    ...
                                    <div
                                        class="mb-3 grid grid-cols-1 gap-3 sm:grid-cols-3"
                                    >
                                        <div>
                                            <label
                                                class="mb-1 block text-xs font-medium"
                                ...
                                >
                                    ...
                                    <div class="mb-3">
                                        ...
                                        >
                                        <FormInput
                                            v-model="companyForm.positions_held"
                                            placeholder="e.g., Truck Driver, Dispatcher"
                                        />
                                    </div>
                                    <div
                                        class="mb-3 space-y-2 rounded-md border border-blue-100 bg-blue-50 p-3"
                                    >
                                        <div class="flex items-start gap-2">
                                            <FormCheck.Input
                                                v-model="
                                                    companyForm.subject_to_fmcsr
                                                "
                                                type="checkbox"
                                                class="mt-0.5 shrink-0"
                                            />
                                            <label
                                                class="cursor-pointer text-xs text-slate-700"
                                                >Were you subject to the Federal
                                                Motor Carrier Safety Regulations
                                                while employed by this
                                                employer?</label
                                            >
                                        </div>
                                        <div class="flex items-start gap-2">
                                            <FormCheck.Input
                                                v-model="
                                                    companyForm.safety_sensitive_function
                                                "
                                                type="checkbox"
                                                class="mt-0.5 shrink-0"
                                            />
                                            <label
                                                class="cursor-pointer text-xs text-slate-700"
                                                >Was this job designated as a
                                                safety sensitive function in any
                                                D.O.T. regulated mode subject to
                                                alcohol and controlled substance
                                                testing requirements as required
                                                by 49 CFR Part 40?</label
                                            >
                                        </div>
                                    </div>
                                    <div class="mb-3">
                                        ...
                                        <select
                                            v-model="
                                                companyForm.reason_for_leaving
                                            "
                                            class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:ring-1 focus:ring-primary focus:outline-none"
                                        >
                                            <option value="">
                                    ...
                                    >
                                        ...
                                        <FormInput
                                            v-model="
                                                companyForm.other_reason_description
                                            "
                                    ...
                                    <div>
                                        <label
                                            class="mb-1 block text-xs font-medium"
                                            >Additional Explanation</label
                                        >
                                        <textarea
                                            v-model="companyForm.explanation"
                                            rows="2"
                                            class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:ring-1 focus:ring-primary focus:outline-none"
                                ...
                                <div
                                    class="sticky bottom-0 flex justify-end gap-3 bg-white pt-2 pb-1"
                                >
                                    <Button
                                        variant="outline-secondary"
                                        @click="showCompanyModal = false"
                    ...
                    <!-- ══════════════════════════════════════════════════
                     ...
                     ══════════════════════════════════════════════════ -->
                    <Dialog
                        :open="showUnemploymentModal"
                        @close="showUnemploymentModal = false"
                    >
                        <Dialog.Panel>
                            <div class="p-6">
                                <div
                                ...
                                >
                                    ...
                                    <button
                                        @click="showUnemploymentModal = false"
                                        class="text-slate-400 hover:text-slate-600"
                                    >
                                        <Lucide icon="X" class="h-5 w-5" />
                                    </button>
                                ...
                                >
                                    ...
                                    <div>
                                        ...
                                        >
                                        <Litepicker
                                            v-model="unemploymentForm.end_date"
                                            :options="lpOptions"
                                        />
                                    </div>
                                </div>
                                <div class="mb-4">
                                    ...
                                    >
                                    <textarea
                                        v-model="unemploymentForm.comments"
                                        rows="3"
                                        class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:ring-1 focus:ring-primary focus:outline-none"
                                        placeholder="Add any relevant details about this unemployment period"
                                    />
                                </div>
                            ...
                            <div class="p-6">
                                ...
                                >
                                    <div>
                                        ...
                                        >
                                        <Litepicker
                                            v-model="relatedForm.start_date"
                                            :options="lpOptions"
                                        />
                                    </div>
                                    <div>
                                ...
                                <div class="mb-4">
                                    ...
                                    >
                                    <FormInput
                                        v-model="relatedForm.position"
                                        placeholder="e.g., Cook, Warehouseman, Carpenter"
                                    />
                                </div>
                                <div class="mb-4">
                                    <label
                ...
                <div v-else-if="currentStep === 11">
                    <h2 class="mb-1 text-lg font-semibold">Company Policies</h2>
                    <p class="mb-6 text-sm text-slate-500">
                        Review the company policy document and provide your
                        consent for each section.
                    </p>

                    <!-- Policy Document -->
                    <div
                        class="mb-6 rounded-xl border border-slate-200 bg-slate-50 p-5"
                    >
                        <h3
                        ...
                        <div class="mb-4">
                            ...
                            <span v-else class="text-sm text-slate-400 italic"
                                >No policy document available</span
                            >
                        </div>
                        <p class="mb-4 text-sm text-slate-500">
                            Please review the company policy document before
                            proceeding.
                        </p>
                        <label class="flex cursor-pointer items-start gap-3">
                            <FormCheck.Input
                                v-model="step11.consent_all_policies_attached"
                                type="checkbox"
                                class="mt-0.5"
                            />
                            <span class="text-sm">
                                <span class="font-bold text-danger">*</span>
                                I agree and consent to all policies attached
                                above.
                            </span>
                        </label>
                    </div>
                    ...
                    >
                        <h3 class="mb-3 text-base font-semibold text-slate-800">
                            Controlled Substances &amp; Alcohol Testing Consent
                        </h3>
                        <div
                            class="mb-4 space-y-2 text-sm leading-relaxed text-slate-700"
                        >
                            <p>
                                I understand that as required by the Federal
                                Motor Carrier Safety Regulations or company
                                policy, all drivers must submit to alcohol and
                                controlled substances testing.
                            </p>
                            <p>
                                I consent to all such testing as a condition of
                                my employment. I understand that if I test
                                positive for illegal drugs or alcohol misuse, I
                                will not be eligible for employment with this
                                company.
                            </p>
                        </div>
                        <label class="flex cursor-pointer items-start gap-3">
                            <FormCheck.Input
                                v-model="step11.substance_testing_consent"
                                type="checkbox"
                                class="mt-0.5"
                            />
                            <span class="text-sm">
                    ...
                    >
                        ...
                        >
                            <p>
                                I authorize you to make such investigations and
                                inquiries of my personal, employment, financial
                                or medical history and other related matters as
                                may be necessary in arriving at an employment
                                decision.
                            </p>
                            <p>
                                I hereby release employers, schools, health care
                                providers and other persons from all liability
                                in responding to inquiries and releasing
                                information in connection with my application.
                            </p>
                        </div>
                        <label class="flex cursor-pointer items-start gap-3">
                            <FormCheck.Input
                                v-model="step11.authorization_consent"
                                type="checkbox"
                                class="mt-0.5"
                            />
                            <span class="text-sm">
                    ...
                    >
                        <h3 class="mb-3 text-base font-semibold text-slate-800">
                            General Consent for Limited Queries of the FMCSA
                        ...
                        >
                            <p>
                                I hereby consent to
                                <strong>{{
                                    step11.company_name || 'the company'
                                }}</strong>
                                conducting limited queries of the Federal Motor
                                Carrier Safety Administration (FMCSA) Commercial
                                Driver's License Drug and Alcohol Clearinghouse
                                to determine whether drug or alcohol violation
                                information about me exists in the
                                Clearinghouse.
                            </p>
                        </div>
                        <div class="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                            ...
                            <div>
                                <label
                                    class="mb-1 block text-xs font-medium text-slate-600"
                                    >Commercial Driver's License Number</label
                                >
                                <div
                                    class="rounded-lg bg-slate-100 px-3 py-2 text-sm text-slate-700"
                                >
                        ...
                        <label class="flex cursor-pointer items-start gap-3">
                            <FormCheck.Input
                                v-model="step11.fmcsa_clearinghouse_consent"
                                type="checkbox"
                                class="mt-0.5"
                            />
                            <span class="text-sm">
                    ...
                    <!-- Validation errors -->
                    <div
                        v-if="step11Errors.length"
                        class="mb-4 rounded-xl border border-red-200 bg-red-50 p-4"
                    >
                        <ul class="list-inside list-disc space-y-1">
                            <li
                                v-for="e in step11Errors"
                                :key="e"
                                class="text-sm text-red-700"
                            >
                                {{ e }}
                ...
                <div v-else-if="currentStep === 12">
                    ...
                    <p class="mb-6 text-sm text-slate-500">
                        Please answer all questions truthfully. A "Yes" answer
                    ...
                    >
                        <h3 class="mb-3 text-base font-semibold text-slate-800">
                            Criminal Record <span class="text-danger">*</span>
                        </h3>
                        ...
                        <div class="flex items-center gap-6">
                            ...
                            >
                                <input
                                    type="radio"
                                    :value="false"
                                    v-model="step12.has_criminal_charges"
                    ...
                    >
                        <h3 class="mb-3 text-base font-semibold text-slate-800">
                            Felonies <span class="text-danger">*</span>
                        </h3>
                        <p class="mb-3 text-sm text-slate-700">
                            Have you ever pled 'guilty' to, been convicted of,
                            or pled 'no contest' to a felony?
                        </p>
                        <div class="mb-5 flex items-center gap-6">
                            ...
                            >
                                <input
                                    type="radio"
                                    :value="false"
                                    v-model="step12.has_felony_conviction"
                                    class="form-radio h-4 w-4 text-primary"
                                />
                                <span class="text-sm text-slate-700">No</span>
                            </label>
                        ...
                        <p class="mb-3 text-sm text-slate-700">
                            If you have any felony convictions, do you currently
                            hold a minister's permit to enter or exit Canada?
                        </p>
                        <div class="flex items-center gap-6">
                            <label
                                class="inline-flex cursor-pointer items-center gap-2"
                            >
                                <input
                                    type="radio"
                                    :value="false"
                                    v-model="step12.has_minister_permit"
                                    class="form-radio h-4 w-4 text-primary"
                                />
                                <span class="text-sm text-slate-700">No</span>
                            </label>
                    ...
                    >
                        ...
                        >
                            <h4 class="font-medium">
                                ...
                                Authorization Form For Employment Purposes
                            </h4>
                            <p>
                                I understand that, pursuant to the federal Fair
                                Credit Reporting Act (FCRA), if any adverse
                                action is to be taken based upon the consumer
                                report, a copy of the report and a summary of
                                the consumer's rights will be provided to me.
                            </p>
                        </div>
                        <label class="flex cursor-pointer items-start gap-3">
                            <FormCheck.Input
                                v-model="step12.fcra_consent"
                                type="checkbox"
                                class="mt-0.5"
                            />
                            <span class="text-sm"
                                ><span class="font-bold text-danger">*</span> I
                                agree and consent to the above</span
                            >
                        </label>
                    </div>
                    ...
                    >
                        ...
                        <div class="mb-4 grid grid-cols-1 gap-4 md:grid-cols-3">
                            <div>
                                ...
                                <div
                                    class="rounded-lg bg-slate-100 px-3 py-2 text-sm text-slate-700"
                                >
                                    {{ step12.full_name || 'Not available' }}
                                </div>
                            </div>
                            <div>
                                <label
                                ...
                                <div
                                    class="rounded-lg bg-slate-100 px-3 py-2 text-sm text-slate-700"
                                >
                                    {{ step12.middle_name || 'Not available' }}
                                </div>
                            </div>
                            <div>
                                <label
                                ...
                                <div
                                    class="rounded-lg bg-slate-100 px-3 py-2 text-sm text-slate-700"
                                >
                                    {{ step12.last_name || 'Not available' }}
                                </div>
                            </div>
                        </div>
                        <div class="mb-4 grid grid-cols-1 gap-4 md:grid-cols-3">
                            <div>
                                ...
                                >
                                    {{
                                        step12.ssn_last_four || 'Not available'
                                    }}
                                </div>
                            </div>
                            <div>
                                ...
                                >
                                    {{
                                        step12.date_of_birth
                                            ? toUsDate(step12.date_of_birth)
                                            : 'Not available'
                                    }}
                                </div>
                            </div>
                            <div>
                                ...
                                >
                                    {{
                                        step12.license_number || 'Not available'
                                    }}
                                </div>
                            </div>
                        </div>
                        <div class="mb-4">
                            <label
                                class="mb-1 block text-xs font-medium text-slate-600"
                                >License State of Issuance</label
                            >
                            <div
                                class="rounded-lg bg-slate-100 px-3 py-2 text-sm text-slate-700"
                            >
                                {{ step12.license_state || 'Not available' }}
                            </div>
                        </div>
                            ...
                            >
                                <table class="w-full text-sm">
                                    ...
                                    <tbody class="divide-y divide-slate-100">
                                        <template
                                            v-if="
                                                props.stepData?.step2?.primary
                                            "
                                        >
                                            <tr class="bg-white">
                                                <td
                                                    class="px-4 py-2 text-slate-600"
                                                >
                                                    {{
                                                        props.stepData.step2
                                                            .primary
                                                            .address_line1
                                                    }}<span
                                                        v-if="
                                        ...
                                        <template
                                            v-for="addr in props.stepData?.step2
                                                ?.previous ?? []"
                                            :key="addr.id"
                                        >
                        ...
                        <label class="flex cursor-pointer items-start gap-3">
                            <FormCheck.Input
                                v-model="step12.background_info_consent"
                                type="checkbox"
                                class="mt-0.5"
                            />
                            <span class="text-sm"
                                ><span class="font-bold text-danger">*</span> By
                                signing below, you are certifying that the above
                                information is true and correct.</span
                            >
                        </label>
                    </div>
                    ...
                    <!-- Validation errors -->
                    <div
                        v-if="step12Errors.length"
                        class="mb-4 rounded-xl border border-red-200 bg-red-50 p-4"
                    >
                        <ul class="list-inside list-disc space-y-1">
                            <li
                                v-for="e in step12Errors"
                                :key="e"
                                class="text-sm text-red-700"
                            >
                                {{ e }}
                ...
                <div v-else-if="currentStep === 13">
                    <h2 class="mb-1 text-lg font-semibold">W-9 Tax Form</h2>
                    <p class="mb-6 text-sm text-slate-500">
                        Complete the W-9 Request for Taxpayer Identification
                    ...
                    >
                        <h3 class="mb-1 text-base font-semibold text-slate-800">
                            Line 1 — Name <span class="text-danger">*</span>
                        </h3>
                        ...
                        />
                        <p
                            v-if="step13Errors.find((e) => e.includes('Name'))"
                            class="mt-1 text-xs text-red-500"
                        >
                            {{ step13Errors.find((e) => e.includes('Name')) }}
                        </p>
                    </div>
                    ...
                    >
                        ...
                        <div
                            class="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3"
                        >
                            <label
                                v-for="[val, lbl] in [
                                    [
                                ...
                                :key="val"
                                class="relative flex cursor-pointer items-start rounded-lg border p-3 transition-all"
                                :class="
                                    step13.tax_classification === val
                            ...
                            >
                                <input
                                    type="radio"
                                    :value="val"
                                    v-model="step13.tax_classification"
                                    class="mt-0.5 form-radio h-4 w-4 border-slate-300 text-primary"
                                />
                        ...
                        <!-- LLC sub-option -->
                        <div
                            v-if="step13.tax_classification === 'llc'"
                        ...
                        >
                            ...
                            <div class="flex items-center gap-6">
                                ...
                                >
                                    <input
                                        type="radio"
                                        :value="v"
                                        v-model="step13.llc_classification"
                                        class="form-radio h-4 w-4 text-primary"
                                    />
                            ...
                            <p
                                v-if="
                                    step13Errors.find((e) => e.includes('LLC'))
                                "
                                class="mt-1 text-xs text-red-500"
                            >
                                {{
                        ...
                        <!-- Other sub-option -->
                        <div
                            v-if="step13.tax_classification === 'other'"
                            class="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-4"
                        >
                            <label
                                class="mb-1 block text-sm font-medium text-slate-700"
                            ...
                            >
                            <FormInput
                                v-model="step13.other_classification"
                            ...
                            <p
                                v-if="
                                    step13Errors.find((e) =>
                                        e.includes('other classification'),
                                    )
                                "
                                class="mt-1 text-xs text-red-500"
                            >
                    ...
                    <div
                        v-if="
                            ['partnership', 'trust_estate'].includes(
                                step13.tax_classification,
                            ) ||
                            (step13.tax_classification === 'llc' &&
                                step13.llc_classification === 'P')
                        "
                        class="mb-6 rounded-xl border border-slate-200 bg-white p-5 shadow-sm"
                    >
                        <h3 class="mb-3 text-base font-semibold text-slate-800">
                        ...
                        <label class="flex cursor-pointer items-start gap-3">
                            <FormCheck.Input
                                v-model="step13.has_foreign_partners"
                                type="checkbox"
                                class="mt-0.5"
                            />
                            <span class="text-sm text-slate-700"
                                >Check this box if you have any foreign
                                partners, owners, or beneficiaries.</span
                            >
                        </label>
                    </div>
                    ...
                    >
                        ...
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                            <div>
                                ...
                                >
                                <FormInput
                                    v-model="step13.exempt_payee_code"
                                    placeholder="1–13"
                                    maxlength="2"
                                    inputmode="numeric"
                                    @input="onExemptCodeInput"
                                />
                            </div>
                            <div>
                                <label
                                ...
                                >
                                <FormInput
                                    v-model="step13.fatca_exemption_code"
                                    placeholder="A–M"
                                    maxlength="1"
                                />
                            </div>
                    ...
                    >
                        ...
                        <FormInput
                            v-model="step13.account_numbers"
                        ...
                        <div class="mb-4 flex items-center gap-6">
                            ...
                            >
                                <input
                                    type="radio"
                                    value="ssn"
                                    v-model="step13.tin_type"
                                    class="form-radio h-4 w-4 text-primary"
                                />
                                <span class="text-sm font-medium text-slate-700"
                                    >Social Security Number (SSN)</span
                                >
                            </label>
                            <label
                                class="inline-flex cursor-pointer items-center gap-2"
                            ...
                            <FormInput
                                ...
                                :placeholder="
                                    step13.tin_type === 'ssn'
                                        ...
                                        : 'XX-XXXXXXX'
                                "
                                :maxlength="step13.tin_type === 'ssn' ? 11 : 10"
                                inputmode="numeric"
                                @input="onTinInput"
                            />
                            <p
                                v-if="
                                    step13Errors.find((e) => e.includes('TIN'))
                                "
                                class="mt-1 text-xs text-red-500"
                            >
                                {{
                    ...
                    <!-- Certification note -->
                    <div
                        class="mb-6 flex items-start gap-3 rounded-xl border border-blue-200 bg-blue-50 p-4"
                    >
                        <Lucide
                            icon="Info"
                        ...
                        <p class="text-sm text-blue-800">
                            The signature for Part II (Certification) will be
                            applied automatically from the Certification step
                    ...
                    <!-- Validation errors -->
                    <div
                        v-if="step13Errors.length"
                        class="mb-4 rounded-xl border border-red-200 bg-red-50 p-4"
                    >
                        <ul class="list-inside list-disc space-y-1">
                            <li
                                v-for="e in step13Errors"
                                :key="e"
                                class="text-sm text-red-700"
                            >
                                {{ e }}
                ...
                <div v-else-if="currentStep === 14">
                    ...
                    <p class="mb-6 text-sm text-slate-500">
                        ...
                        me, and that all entries on it and information in it are
                    ...
                    >
                        <h3 class="mb-2 text-base font-semibold text-slate-800">
                            Safety Performance History Investigation — Previous
                            USDOT Regulated Employers
                        </h3>
                        <p class="mb-4 text-sm text-slate-600">
                            I hereby specifically authorize you to release the
                            following information to the specified company and
                            their agents for the purposes of investigation as
                            required by §391.23 and §40.321(b) of the Federal
                            Motor Carrier Safety Regulations. You are hereby
                            released from any and all liability which may result
                        ...
                        >
                            <table class="w-full text-sm">
                                ...
                                <tbody class="divide-y divide-slate-100">
                                    ...
                                    <tr
                                        v-if="!step14.employment_history.length"
                                    >
                                        <td
                                            colspan="7"
                                            class="px-4 py-4 text-center text-slate-400"
                    ...
                    >
                        ...
                        <p class="mb-4 text-sm text-slate-600">
                            By signing below, I agree to use an electronic
                            signature and acknowledge that an electronic
                        ...
                        <div class="mb-3 flex items-center justify-between">
                            ...
                            >
                                <Lucide icon="PenLine" class="mr-1 h-4 w-4" />
                                {{
                                    step14.signature
                                        ? 'Change Signature'
                                        : 'Sign Now'
                        ...
                        <!-- Signature Preview -->
                        <div
                            v-if="step14.signature || step14.signature_url"
                        ...
                        >
                            ...
                            <div
                                class="flex justify-center rounded border border-slate-200 bg-white p-3"
                            >
                                <img
                                    :src="
                                        step14.signature || step14.signature_url
                                    "
                                    alt="Signature"
                                    class="max-h-32"
                                />
                        ...
                        <div
                            v-else
                            class="rounded-lg border border-dashed border-slate-300 bg-slate-50 p-6 text-center text-sm text-slate-400"
                        >
                            No signature provided. Click "Sign Now" to add your
                            signature.
                        </div>
                    </div>
                    ...
                    >
                        <label class="flex cursor-pointer items-start gap-3">
                            <FormCheck.Input
                                v-model="step14.is_accepted"
                                type="checkbox"
                                class="mt-0.5"
                            />
                            <span class="text-sm">
                                <span class="font-bold text-danger">*</span>
                                I hereby certify that all information provided
                                in this application is true and complete to the
                    ...
                    <!-- Validation errors -->
                    <div
                        v-if="step14Errors.length"
                        class="mb-4 rounded-xl border border-red-200 bg-red-50 p-4"
                    >
                        <ul class="list-inside list-disc space-y-1">
                            <li
                                v-for="e in step14Errors"
                                :key="e"
                                class="text-sm text-red-700"
                            >
                                {{ e }}
                    ...
                    <div class="mt-6 flex justify-between">
                        <Button
                            variant="outline-secondary"
                            @click="currentStep = 13"
                        >
                            <Lucide icon="ChevronLeft" class="mr-1 h-4 w-4" />
                            Previous
                        </Button>
                        <Button variant="success" @click="submitStep14">
                            <Lucide icon="CheckCircle" class="mr-1 h-4 w-4" />
                    ...
                    <Teleport to="body">
                        ...
                        >
                            ...
                            >
                                ...
                                <div class="p-5">
                                    <div
                                        class="mb-4 rounded-lg border-2 border-slate-300 bg-white"
                                        style="height: 256px"
                                    >
                                        <canvas
                                            ref="signatureCanvas"
                                            class="h-full w-full cursor-crosshair rounded-lg"
                                        ></canvas>
                                    </div>
                                    <div class="flex justify-end gap-3">
                                        <Button
                ...
                <div v-else-if="currentStep === 15">
                    <div class="space-y-6">
                                    ...
                                    <div>
                                        ...
                                        <p class="mt-1 text-sm text-slate-500">
                                            <template v-if="step15.is_complete">
                                                The driver file is complete and
                                                ready for final confirmation.
                                            </template>
                                            <template v-else>
                                                Review the pending sections and
                        ...
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                            ...
                            >
                                ...
                                <p
                                    class="mt-1 text-3xl font-semibold text-primary"
                                >
                                    {{ Math.round(step15.total_percentage) }}%
                                </p>
                            ...
                            <div
                                class="box box--stacked border border-warning/10 bg-warning/5 p-5"
                            >
                                <p class="text-sm text-slate-500">
                                    Pending Steps
                                </p>
                                <p
                                    class="mt-1 text-3xl font-semibold text-warning"
                                >
                                    {{ step15.steps_needing_attention.length }}
                                </p>
                                <p class="mt-1 text-xs text-slate-500">
                                    Sections still need review
                                </p>
                            </div>
                            <div
                                class="box box--stacked p-5"
                                :class="
                                    step15.is_complete
                                        ? 'border border-success/10 bg-success/5'
                        ...
                        <div
                            class="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1.15fr)_minmax(320px,0.85fr)]"
                        >
                            <div class="space-y-6">
                                <div
                                    v-if="
                                ...
                                >
                                    <div
                                        class="mb-5 flex items-start gap-3 border-b border-slate-200 pb-4"
                                    >
                                        <div
                                            class="rounded-xl border border-warning/20 bg-warning/10 p-2.5"
                                        >
                                            <Lucide
                                                icon="AlertTriangle"
                                                class="h-5 w-5"
                                        ...
                                        <div class="min-w-0 flex-1">
                                            ...
                                            >
                                                The following sections still
                                                have missing required
                                                information.
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="space-y-3">
                                        <button
                                            ...
                                            :key="s.step"
                                            type="button"
                                            class="group flex w-full flex-col gap-3 rounded-xl border border-slate-200 bg-slate-50/70 p-4 text-left transition hover:border-primary/30 hover:bg-white sm:flex-row sm:items-center sm:justify-between"
                                        ...
                                        >
                                            ...
                                            >
                                                <div
                                                    class="h-2.5 w-full min-w-[120px] overflow-hidden rounded-full bg-slate-100 sm:w-32"
                                                >
                                                    <div
                                ...
                                <div
                                    v-else
                                    class="box box--stacked border border-success/10 bg-success/5 p-6"
                                >
                                    <div class="flex items-start gap-3">
                                        <div
                                            class="rounded-xl border border-success/20 bg-success/10 p-2.5"
                                        >
                                            <Lucide
                                                icon="CheckCircle2"
                                                class="h-5 w-5"
                                        ...
                                        <div>
                                            <h3
                                                class="text-base font-semibold text-slate-800"
                                            >
                                                All required steps look complete
                                            </h3>
                                            <p
                                                class="mt-1 text-sm text-slate-600"
                                            >
                                                No pending sections were
                                                detected. You can finalize the
                                                registration once you've
                                                reviewed the notice on the
                                                right.
                                            </p>
                                        </div>
                                    </div>
                                ...
                                <div class="box box--stacked p-6">
                                    <div class="flex items-start gap-3">
                                        ...
                                        <div class="min-w-0 flex-1">
                                            ...
                                            >
                                                <template
                                                    v-if="step15.is_complete"
                                                >
                                                    The driver registration has
                                                    been completed successfully.
                                                    All information has been
                                                    saved and is ready for final
                                                    confirmation.
                                                </template>
                                                <template v-else>
                                                    Please complete the required
                                                    fields in the highlighted
                                                    sections before trying to
                            ...
                            <div class="space-y-6">
                                <div class="box box--stacked overflow-hidden">
                                    <div
                                        class="flex items-center gap-3 border-b border-slate-200 bg-slate-50/80 p-5"
                                    >
                                        <div class="flex items-center gap-3">
                                            <div
                                    ...
                                    <div class="space-y-4 p-5">
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                Clearinghouse Electronic Consent
                                                Required
                                            </p>
                                            <p
                                                class="mt-2 text-sm leading-6 text-slate-600"
                                            >
                                                ...
                                                <strong
                                                    >electronic consent</strong
                                                >
                                                for a prospective employer to
                                                view their information in the
                                                FMCSA's Drug and Alcohol
                                                Clearinghouse.
                                            </p>
                                        </div>
                                        ...
                                        <div class="space-y-3">
                                            <div
                                                class="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50/80 p-3"
                                            >
                                                <div
                                                ...
                                                >
                                                    The driver should register
                                                    in the FMCSA Clearinghouse
                                                    portal using the official
                                                    link below.
                                                </p>
                                            </div>
                                            <div
                                            ...
                                            >
                                                ...
                                                >
                                                    ...
                                                    driver must grant the
                                                    electronic consent from that
                                                    account.
                                                </p>
                                            </div>
                                            <div
                                        ...
                                        <a
                                            ...
                                            target="_blank"
                                            class="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-primary px-4 py-3 text-sm font-semibold text-white transition hover:bg-primary/90"
                                        >
                                            <Lucide
                                                icon="ExternalLink"
                                                class="h-4 w-4"
                    ...
                    <div class="mt-6 flex justify-between">
                        ...
                        <Button
                            :variant="
                                step15.is_complete ? 'success' : 'secondary'
                            "
