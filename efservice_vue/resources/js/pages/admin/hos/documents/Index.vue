...
function documentTypeMeta(type: string) {
    return (
        typeConfig[type] ?? {
            icon: 'FileText',
...
function fileIcon(fileName: string) {
    ...
    if (/\.(doc|docx)$/.test(lower)) return 'FileBadge';
...
function applyFilters() {
    router.get(
        route(props.routeNames?.index ?? 'admin.hos.documents.index'),
        {
...
function generateDailyLog() {
    router.post(
        route(
            props.routeNames?.generateDailyLog ??
                'admin.hos.documents.generate-daily-log',
        ),
        dailyForm,
        { preserveScroll: true },
    );
...
function generateMonthlySummary() {
    router.post(
        route(
            props.routeNames?.generateMonthlySummary ??
                'admin.hos.documents.generate-monthly-summary',
        ),
        monthlyForm,
        { preserveScroll: true },
    );
...
function generateFmcsaMonthly() {
    router.post(
        route(
            props.routeNames?.generateFmcsaMonthly ??
                'admin.hos.documents.generate-fmcsa-monthly',
        ),
        fmcsaForm,
        { preserveScroll: true },
    );
...
function destroyDocument(id: number) {
    ...
    router.delete(
        route(props.routeNames?.destroy ?? 'admin.hos.documents.destroy', id),
        { preserveScroll: true },
    );
}
...
function bulkDelete() {
    if (
        ...
        !confirm(`Delete ${selectedDocuments.value.length} selected documents?`)
    )
        return;
    router.post(
        route(
            props.routeNames?.bulkDestroy ?? 'admin.hos.documents.bulk-destroy',
        ),
...
function bulkDownload() {
    ...
    window.location.href = route(
        props.routeNames?.bulkDownload ?? 'admin.hos.documents.bulk-download',
        { ids: selectedDocuments.value.join(',') },
    );
}
</script>
...
<template>
    ...
    <div class="grid grid-cols-12 gap-6">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-sm text-slate-500">
                                Generate and manage daily logs, monthly
                                summaries, FMCSA monthlies, and trip reports.
                            </p>
                        </div>
                    </div>
        ...
        <div
            class="col-span-12 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-6"
        >
            <div
                class="box box--stacked border border-primary/10 bg-primary/[0.04] p-5"
            >
            ...
            <div
                class="box box--stacked border border-info/10 bg-info/[0.04] p-5"
            >
                <div class="text-sm text-slate-500">Trip Reports</div>
                <div class="mt-2 text-3xl font-semibold text-info">
                    {{ stats.trip_reports }}
                </div>
            </div>
            <div
                class="box box--stacked border border-warning/10 bg-warning/[0.04] p-5"
            >
                <div class="text-sm text-slate-500">Inspection Reports</div>
                <div class="mt-2 text-3xl font-semibold text-warning">
                    {{ stats.inspection_reports }}
                </div>
            </div>
            <div
                class="box box--stacked border border-primary/10 bg-primary/[0.04] p-5"
            ...
            <div
                class="box box--stacked border border-success/10 bg-success/[0.04] p-5"
            >
                <div class="text-sm text-slate-500">Monthly Summaries</div>
                <div class="mt-2 text-3xl font-semibold text-success">
            ...
            <div
                class="box box--stacked border border-info/10 bg-info/[0.04] p-5"
            >
                <div class="text-sm text-slate-500">FMCSA Monthly</div>
                <div class="mt-2 text-3xl font-semibold text-info">
                    {{ stats.fmcsa_monthly }}
                </div>
            </div>
            ...
            <div class="box box--stacked p-5">
                ...
                >
                    ...
                    <TomSelect v-model="filters.type">
                        ...
                        <option value="inspection_reports">
                            Inspection Reports
                        </option>
                        <option value="daily_logs">Daily Logs</option>
                        <option value="monthly_summaries">
                            Monthly Summaries
                        </option>
                    ...
                    <table class="min-w-full divide-y divide-slate-200 text-sm">
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white">
                            ...
                            >
                                ...
                                <td class="px-5 py-4">
                                    <div class="flex items-center gap-3">
                                        <img
                                            v-if="document.driver_avatar"
                                            :src="document.driver_avatar"
                                            :alt="document.driver_name"
                                ...
                                <td class="px-5 py-4">
                                    <span
                                        ...
                                        :class="
                                            documentTypeMeta(document.type)
                                                .classes
                                        "
                                    >
                                        <Lucide
                                ...
                                <td class="px-5 py-4">
                                    <div class="flex items-start gap-3">
                                        <div
                                            class="mt-0.5 flex h-10 w-10 items-center justify-center rounded-xl bg-slate-100 text-slate-500"
                                        >
                                            <Lucide
                                                :icon="
                                ...
                                <td class="px-5 py-4 text-right">
                                    <div class="flex justify-end gap-2">
                                        <a
                                            ...
                                            target="_blank"
                                            class="inline-flex items-center gap-1.5 rounded-lg border border-primary/20 bg-primary/5 px-3 py-1.5 text-xs font-medium text-primary transition hover:bg-primary/10"
                                        >
                                            <Lucide
                                                icon="Eye"
                                                class="h-3.5 w-3.5"
                                        ...
                                        <a
                                            ...
                                            class="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-medium text-slate-600 transition hover:border-primary/30 hover:text-primary"
                                        >
                                            <Lucide
                                                icon="Download"
                                                class="h-3.5 w-3.5"
                                        ...
                                        <button
                                            class="inline-flex items-center gap-1.5 rounded-lg border border-danger/20 bg-danger/5 px-3 py-1.5 text-xs font-medium text-danger transition hover:bg-danger/10"
                                            @click="
                            ...
                            <tr v-if="!documents.length">
                                ...
                                >
                                    No HOS documents matched the current
                                    filters.
                                </td>
                            </tr>
                        </tbody>
