...
const props = defineProps<{ driver: any; inspections: any; vehicles: any[]; inspectionTypes: string[]; statuses: string[]; filters: any }>()
const filters = reactive({ ...props.filters })
function applyFilters() {
    router.get(route('admin.inspections.driver-history', props.driver.id), {
...
<template>
    ...
    <div class="grid grid-cols-12 gap-x-6 gap-y-10">
        <div class="col-span-12">
            <div class="box box--stacked p-6 mb-6"><div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4"><div class="flex items-center gap-4"><div class="p-3 bg-primary/10 rounded-xl border border-primary/20"><Lucide icon="History" class="w-8 h-8 text-primary" /></div><div><h1 class="text-2xl font-bold text-slate-800">Inspection History</h1><p class="text-slate-500">All inspections for {{ driver.name }}</p></div></div><div class="flex flex-wrap items-center gap-3"><Link :href="route('admin.drivers.show', driver.id)" class="inline-flex items-center gap-2 px-4 py-2 border border-slate-300 rounded-lg text-slate-600 hover:bg-slate-50 transition"><Lucide icon="ArrowLeft" class="w-4 h-4" /> Back to Driver</Link><Link :href="route('admin.inspections.create')" class="inline-flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary/90 transition"><Lucide icon="Plus" class="w-4 h-4" /> Add Inspection</Link></div></div></div>
            ...
            <div class="box box--stacked p-0 overflow-hidden"><div class="overflow-x-auto"><table class="w-full text-left"><thead><tr class="bg-slate-50/80"><th class="px-5 py-3 text-xs font-medium text-slate-500 uppercase">Date</th><th class="px-5 py-3 text-xs font-medium text-slate-500 uppercase">Vehicle</th><th class="px-5 py-3 text-xs font-medium text-slate-500 uppercase">Type</th><th class="px-5 py-3 text-xs font-medium text-slate-500 uppercase">Status</th><th class="px-5 py-3 text-xs font-medium text-slate-500 uppercase text-center">Actions</th></tr></thead><tbody><tr v-for="inspection in inspections.data" :key="inspection.id" class="border-b border-slate-100 hover:bg-slate-50/50 transition"><td class="px-5 py-4 text-sm text-slate-600">{{ inspection.inspection_date_display }}</td><td class="px-5 py-4 text-sm text-slate-600">{{ inspection.vehicle?.label ?? 'N/A' }}</td><td class="px-5 py-4 text-sm text-slate-700">{{ inspection.inspection_type }}</td><td class="px-5 py-4 text-sm text-slate-600">{{ inspection.status ?? 'N/A' }}</td><td class="px-5 py-4"><div class="flex items-center justify-center gap-2"><Link :href="route('admin.inspections.edit', inspection.id)" class="p-1.5 text-slate-400 hover:text-amber-500 transition"><Lucide icon="PenLine" class="w-4 h-4" /></Link><Link :href="route('admin.inspections.driver-documents', driver.id)" class="p-1.5 text-slate-400 hover:text-primary transition"><Lucide icon="Files" class="w-4 h-4" /></Link></div></td></tr><tr v-if="!inspections.data.length"><td colspan="5" class="px-5 py-12 text-center text-slate-400"><Lucide icon="FileCheck" class="w-12 h-12 mx-auto mb-3 text-slate-300" /><p>No inspections found for this driver</p></td></tr></tbody></table></div></div>
        </div>
    </div>
</template>
