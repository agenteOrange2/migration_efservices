...
const props = defineProps<{ driver: any; documents: any; filters: any }>()
const filters = reactive({ ...props.filters })

function applyFilters() {
    router.get(route('admin.inspections.driver-documents', props.driver.id), {
        ...
        <div class="col-span-12">
            <div class="box box--stacked p-6 mb-6">
                <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
                    ...
                    <div class="flex flex-wrap items-center gap-3"><Link :href="route('admin.inspections.documents.index')" class="inline-flex items-center gap-2 px-4 py-2 border border-slate-300 rounded-lg text-slate-600 hover:bg-slate-50 transition"><Lucide icon="Files" class="w-4 h-4" /> All Documents</Link><Link :href="route('admin.inspections.index')" class="inline-flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary/90 transition"><Lucide icon="ArrowLeft" class="w-4 h-4" /> All Inspections</Link></div>
                </div>
            </div>
