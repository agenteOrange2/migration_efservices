                ...
                <div v-if="documents.data.length" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    <div v-for="document in documents.data" :key="document.id" class="border rounded-lg overflow-hidden shadow-sm">
                        ...
                        <div class="p-4 text-sm text-slate-600 space-y-2">
                            <div><span class="text-xs font-medium text-slate-500">Inspection:</span> <Link v-if="document.inspection_id" :href="route(routeName('edit'), document.inspection_id)" class="text-primary hover:underline">{{ document.inspection_type }} ({{ document.inspection_date_display }})</Link></div>
                            <div><span class="text-xs font-medium text-slate-500">Driver:</span> <Link v-if="document.driver_id" :href="route(routeName('driverDocuments'), document.driver_id)" class="text-primary hover:underline">{{ document.driver_name }}</Link></div>
                            <div><span class="text-xs font-medium text-slate-500">Carrier:</span> {{ document.carrier_name ?? props.carrier?.name }}</div>
