...
function markAsRead(notification: NotificationItem) {
    router.post(route('admin.notifications.mark-as-read', notification.id), {}, {
        preserveScroll: true,
    })
}

function markAsUnread(notification: NotificationItem) {
    router.post(route('admin.notifications.mark-as-unread', notification.id), {}, {
        preserveScroll: true,
    })
}
...
function deleteFiltered() {
    ...
    router.visit(route('admin.notifications.delete-all'), {
        method: 'delete',
        data: {
            search: filters.search || undefined,
            status: filters.status || undefined,
                ...
                <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-slate-500">Review all activity, clear unread items, and keep the admin team on top of alerts without relying only on the bell.</p>
                        </div>
                    </div>
