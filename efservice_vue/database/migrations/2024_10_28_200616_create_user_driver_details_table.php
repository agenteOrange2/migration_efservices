...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('user_driver_details', function (Blueprint $table) {
            ...
            $table->string('hos_cycle_type', 10)->default('70_8')->comment('HOS weekly cycle type: 60_7 or 70_8');
            $table->boolean('hos_cycle_change_requested')->default(false)->comment('True if driver has a pending cycle change request');
            $table->string('hos_cycle_change_requested_to', 10)->nullable()->comment('Requested new cycle type (60_7 or 70_8)');
            $table->timestamp('hos_cycle_change_requested_at')->nullable()->comment('When the cycle change was requested');
            $table->timestamp('hos_cycle_change_approved_at')->nullable()->comment('When the cycle change was approved');
            $table->foreignId('hos_cycle_change_approved_by')->nullable()->constrained('users')->onDelete('set null')->comment('User who approved the cycle change');
